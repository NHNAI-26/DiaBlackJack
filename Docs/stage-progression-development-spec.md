...
> 문서 책임자: 이천서  
> 버전: v0.3
> 상태: SP-04 구현·검증 완료 기준안 · 정식 런 무카드보상 RF 경계 명시
> 최종 갱신: 2026-07-30
...
SP-04 최초 구현은 전투 승리 직후 일반전이면 `StageCleared`, 최종 보스면 `RunVictory`로 전환했고, 이후 RW 구현이 모든 승리를 `RewardSelection`으로 연결했다. 2026-07-30 개정 정식 런은 기존 RW 타입을 삭제하지 않은 채 명시적 무카드보상 경계로 일반전 `StageCleared`와 보스 `RunVictory`를 직접 완료한다. 이 문서의 기존 SP·RW 검증 결과는 당시 기준선으로 보존하고 새 흐름 구현 완료로 소급하지 않는다.

Notion v0.7은 적 프로필 수와 전투 데이터를 바꾸지만 `RunProgress`의 순수 상태 전이를 바꾸지 않는다. 선택된 여섯 프로필 키의 전투 생성은 EP-R04, 상점·골드 게이트는 RF에서 확장하며 SP-00~SP-04 완료로 소급하지 않는다.
...
| 날짜 | 작성자 | 변경 내용 |
| --- | --- | --- |
| 2026-07-30 | 이천서 | 현행 코드의 `RewardSelection`과 개정 정식 런의 무카드보상 완료를 구분하고, 일반전은 골드·상점, 보스는 즉시 승리로 연결하는 RF 확장 경계를 추가했다. 기존 SP·RW 검증 기록은 보존했다. |
