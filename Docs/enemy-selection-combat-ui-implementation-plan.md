...
| EUI-R01 | Notion v0.7 영혼·능력·계약 단계 표시 재연동 | EP-R 선행 대기 | 개정 6종 미리보기·선택·실제 전투 일치, 두 해상도·전체 회귀 |

EUI-R01은 프로필 값을 UI에 복제하지 않는다. EP-R04~R06에서 `EnemyCombatProfileCatalog`와 안전 표시 스냅샷이 확정된 뒤 Presenter·화면·반복 검증만 수행한다.
