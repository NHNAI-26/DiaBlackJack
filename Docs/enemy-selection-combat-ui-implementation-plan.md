...
> 버전: v0.3
> 현재 단계: EUI-05 과거 완료 · EP-R03 문서 이관 완료
> 다음 단계: EP-R04~R06 뒤 EUI-R01 개정 미리보기·전투 정보 재검증
> 최종 갱신: 2026-07-29
...
| EUI-05 | 반복 회귀·씬·화면·문서 마감 | 완료 | 신규 5/5·StageProgression 122/122·CoreLoop 193/193·전체 315/315·실제 런·두 씬·두 해상도·Console 0 |
| EUI-R01 | Notion v0.7 영혼·능력·계약 단계·예상 골드 표시 재연동 | EP-R 선행 대기 | 개정 6종 미리보기·선택·실제 전투·3·4·6·7·9·15 골드 일치, 두 해상도·전체 회귀 |

EUI-R01은 프로필 값이나 골드를 UI에 복제하지 않는다. EP-R04~R06과 RF-01B에서 카탈로그·안전 표시 스냅샷이 확정된 뒤 Presenter·화면·반복 검증만 수행한다.
...
- `OpponentSelectionSystemValidationTests` 5개가 후보 조합, 양쪽 등급, 두 번의 선택과 고정 보스, 오래된 제안·중복 확정·집중/예고 상태 격리, 기존 고정 세션·독립 전투 호환을 각각 반복 검증했다.
- 신규 5/5, StageProgression 122/122, CoreLoop 193/193, 전체 EditMode 315/315를 통과했다.
- 실제 Play Mode에서 `StageTest`→`CoreLoopTest` 왕복으로 두 선택 전투, 일반/높은 등급 보상, 고정 보스 보상, `RunVictory`, 첫 제안 재시작과 덱 20장 복구를 확인했다.
- 선택·일반·엘리트·보스 화면은 1280×720과 1920×1080에서 겹침·잘림·입력 누락이 없었다.
- `StageTest`와 `CoreLoopTest`는 누락 스크립트·깨진 프리팹 0이며, 실행 전 Console을 비운 뒤 최종 Error/Warning 0을 확인했다.
- 계획의 “실제 전체 흐름 10회”는 자동화된 결정적 전체 흐름 10회로 수행하고, 실제 Play Mode 씬 왕복은 1회 수행해 역할을 분리했다.
- 씬·프리팹·Packages·외부 에셋은 변경하지 않았다.
...
| 날짜 | 작성자 | 변경 내용 |
| --- | --- | --- |
| 2026-07-29 | 이천서 | EP-R04~R06과 RF-01B 뒤 EUI-R01에서 확정 행동·골드·보스 단계 정보를 재연동하는 선행 순서와 검증 범위를 구체화 |
...
| 2026-07-20 | 이천서 | EUI-05 후보·전투·보상·고정 보스·재시작과 상태 격리 반복 검증을 추가하고 신규 5/5·StageProgression 122/122·CoreLoop 193/193·전체 315/315·실제 씬 왕복·두 해상도·Console 0으로 전체 계획을 완료 처리 |
