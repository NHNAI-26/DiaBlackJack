...
> 사기꾼의 체인지 상태, 집행자의 주입 독극물, 적 활성 계약과 보스 계약 단계는 전투 중 상태라 스냅샷 필드가 아니다. 저장된 상대 `BattleProfileKey`와 결정적 시드로 전투를 다시 생성한다. RF-01A에서 골드 저장, RF-01B에서 지급, RF-02에서 상점 상태, RF-03에서 정식 순서를 구현했다. 정산·상점 완료 체크포인트는 RF-04/05 뒤 SV-06에 남아 있다.
...
| RF 골드·상점·사건 | 골드 상태·캡처·복원·프로필별 승리 정산·상점·정식 순서 도메인 구현 | RF-04/05 뒤 상점 안정 체크포인트 연결 |
...
현행 기본 새 런은 사탄·벨페고르·바알제붑·마몬 4장 악마 덱으로 바로 시작하므로 시작 악마 예약 파일과 선택 직후 체크포인트를 만들지 않는다. `RunSaveFlow`는 시작 선택이 없는 세션을 즉시 활성화하고 오래된 예약을 제거한다. 시작 악마 후보 2장과 확정 1장을 쓰는 체크포인트는 빈 악마 덱과 선택 생성기를 명시한 해금 풀·호환 경로에서만 유지한다.

저장 시스템은 HONG의 내부 `RunFlow`·`Shop` 필드를 직접 읽지 않는다. RF 쪽에서 다음 읽기 전용 정보를 제공해야 한다.

- 현재 골드
- 상점 방문 ID와 완료 여부
- 라이터·위스키 공통 서비스 가격 단계
...
| SV04-I05 | 파일 쓰기 실패가 다음 진입을 막고 같은 스냅샷 재시도로 복구된다 |
| SV04-I06 | 같은 시작 체크포인트 반복 복원이 같은 다음 상대 후보를 만든다 |
| SV04-I07 | 영혼 0 패배가 이어하기 불가 터미널 체크포인트를 한 번 기록한다 |
| SV04-I08 | 최종 보스 보상 완료가 승리 터미널 체크포인트를 한 번 기록한다 |
...
| SV06-I04 | 서비스 이용 방문 뒤 저장·복원해 다음 상점의 라이터·위스키 가격 단계가 함께 유지된다 |
...
| 날짜 | 작성자 | 변경 |
| --- | --- | --- |
| 2026-07-30 | 이천서 | `RunSaveFlow`의 공개 생성 경로에 GUID 기반 루트 시드 공급을 추가하고 테스트용 내부 경로에는 `Func<int>` 주입을 유지한다. 신규 시드는 예약·`RunSaveCoordinator`에 전달하며 복원은 저장된 `RootSeed`만 사용하도록 명세 |
