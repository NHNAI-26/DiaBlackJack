...
현행 목표의 기본 새 런은 빈 악마 덱에서 사탄·벨페고르·바알제붑·마몬 풀의 서로 다른 2장을 자동 발급하고 지급 완료 체크포인트를 기록한다. `RunSaveFlow`는 저장 성공 뒤 세션을 활성화하며, 저장 실패 시 같은 지급 결과로 재시도한다. 기존 4장 직접 시작과 후보 2장·확정 1장 경로는 과거 구현으로만 보존한다.
