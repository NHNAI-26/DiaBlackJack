...
CU-05 신규 8개를 포함해 관련 Presenter·진행 통합 28/28, CoreLoop 117/117, StageProgression 34/34와 전체 EditMode 151/151이 통과했다. `StageTest`에서 런을 시작해 `CoreLoopTest`의 같은 전투 인스턴스에 연결되는 것을 확인했다. Game View에서는 수정 구슬의 3개 선택, 해머 비용 선택, 자동 권총의 1~10 선언, 군용 나이프 즉시 결과와 최근 결과·입력 잠금 해제를 확인했다. 양쪽 씬의 누락 스크립트·깨진 프리팹·기타 문제와 최종 Console Error/Warning은 0이었다. 즉시 모드 UI로 연결했으므로 씬 직렬화, 패키지와 외부 에셋은 변경하지 않았다.

### 2.6 CU-06 검증 결과

2026-07-20에 검증 전용 `CardUseSystemValidationTests` 5개를 추가했다. 독립 전투의 자동 권총 승리와 재시작, 수정 구슬 후보 선택의 카드 소유권, 상대 비공개 숫자 1~10의 표시·결과 은닉, 카드 효과에 의한 런 승리·패배와 재시작을 각각 10회 반복했다. 매 반복에서 덱 가용 카드와 사용 중 카드 합계, 손패 ID 고유성, 새 전투 인스턴스, 지속 영혼과 사용 상태 초기화를 확인했다.

신규 5/5, CoreLoop 122/122, StageProgression 34/34, 전체 EditMode 156/156이 통과했다. 실제 `StageProgressionController`·`StageProgressionRuntime`·`CoreLoopController` 경로에서도 군용 나이프 최종 승리와 수정 구슬 버스트 패배를 만들고, `StageTest` 결과 화면의 재시작을 거쳐 `CoreLoopTest`에 새 전투·초기 영혼·`Available` 카드가 생성되는 것을 확인했다. `CoreLoopTest`와 `StageTest`는 각각 문제 0, 최종 Console Error/Warning 0이었다. 런타임·씬·패키지·외부 에셋은 변경하지 않았다.
