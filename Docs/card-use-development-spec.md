...
> 기획·개발 책임자: 이천서  
> 버전: v0.10
> 상태: CU-06·CU-M09 완료 · EP-R04 아스모데우스 7 이하 제한 이관 완료
> 최종 갱신: 2026-07-30

> **현행 기획 이관 결과(2026-07-22):** 위협용 해머의 대상을 상대 공개 카드 1장으로 변경하고 자신의 카드 비용을 제거했다. 아래 CU-04 최초 구현 결과 중 비용 설명은 역사 기록이며, 현행 기술 기준은 2.7·5.2·9.2·13.4를 따른다.

> **중간 버스트 이관 결과(2026-07-22):** 히트·강제 히트·카드 효과 도중의 숫자 버스트는 공개 합으로만 판정하고, 양쪽 스탠드 뒤 최종 승부만 전체 합을 사용한다. 보위 나이프는 비버스트 시 방금 뽑은 카드를 반드시 버린다. 아래 CU-04의 유지 정책 설명은 역사 기록이며 현행 기준은 2.8·9.4·13.4를 따른다.

> **표시 순서 보정 결과(2026-07-22):** CU-M03에서 규칙 손패 순서와 카드 ID는 유지한 채 GameScene 표시 투영만 조정해 양측 비공개 카드가 모두 플레이어 화면 왼쪽에 보이도록 했다. 이 변경은 카드 판정·AI 관측·정보 은닉 규칙을 바꾸지 않는다.

> **비공개 역할 개정 결과(2026-07-29):** CU-M04에서 `IsFaceUp`과 비공개 역할을 분리했다. 비공개 카드는 사용·효과로 앞면 공개되어도 공개 합·중간 버스트·공개 카드 대상에서 제외되고 최종 승부까지 비공개 카드 한 장으로 유지된다. 공개된 숫자는 `KnownPlayerHiddenCardRank`로만 적 관측에 전달하며, 사용 가능한 리볼버 후보가 있으면 모든 적 정책보다 우선해 해당 숫자를 선언한다.

> **적 카드 호버 보정 결과(2026-07-29):** CU-M05에서 `GameScenePresenter`가 앞면 공개된 적 카드에만 숫자·이름·효과 설명과 사용 불가 상태의 호버 배지 허용값을 제공한다. 아직 뒷면인 적 비공개 카드는 순위 0·빈 이름·빈 설명·배지 비허용 상태를 유지하며, `CardView`도 `RevealRank == false`이면 배지 문자열을 만들지 않는다.

> **해머 직접 선택 결과(2026-07-30):** CU-M09에서 GameScene의 해머 선택 옵션을 상대 공개 `CardView`에 투영했다. HUD는 안내 문구만 유지하며 유효 대상 클릭이 기존 선택 API로 이어진다. 다른 선택형 효과의 버튼과 비공개 역할 제외 규칙은 유지한다.

> **Notion v0.7 계약 제한 이관(2026-07-29):** EP-R04에서 아스모데우스 소유자의 수동 카드 제한을 숫자 7 이하로 변경했다. 플레이어·적 가용성은 공통 `CardUseAvailability`를 사용하며 6·7 제한과 8 허용 경계를 테스트했다. CU-M07 레비아탄 흐름은 변경하지 않았다.
...
2026-07-19에 기본 효과 처리기 목록에 `AutoPistolEffectHandler`를 등록했다. 숫자 7·8 리볼버은 정상 라운드의 상대 비공개 카드 한 장을 대상으로 1~10 선택지를 제공하며, 선언 값과 실제 숫자의 비교는 규칙 계층 내부에서만 수행한다. 두 장 이상의 비공개 카드를 위한 기능은 추가하지 않았고, 대상 카드가 없으면 승인 전에 상태 변경 없이 거절한다.
...
보위 나이프 9·10은 상대 공개 합계가 16 이하이고 덱이 한 장 이상일 때만 공개 강제 드로우를 수행한다. 버스트가 아니면 교체 가능한 유지 정책에 결과를 위임하며, 현재 프로토타입 정책은 항상 유지한다. CU-04 전용 18/18, CoreLoop 113/113, 전체 EditMode 143/143이 통과했다. 이번 단계는 순수 규칙·CoreLoop 세션 범위이므로 View, `StageProgressionSession`, 씬, 패키지와 외부 에셋은 변경하지 않았으며 실제 화면 사용은 CU-05에서 연결한다.

### 2.5 CU-05 구현 결과

2026-07-19에 `CoreLoopViewModel`을 플레이어 카드별 ID·숫자·표시 이름·사용 상태·사용 가능 여부·불가 사유, 보류 효과 안내·선택지와 최근 공개 효과 결과까지 확장했다. Presenter는 기존 전투 규칙의 `CardUseAvailability`만 표시 데이터로 변환하며 상대 비공개 숫자는 읽거나 노출하지 않는다.

`CoreLoopView`에는 카드별 사용 버튼과 효과 선택 전용 화면을 추가했다. 선택 중에는 일반 행동과 다른 카드 입력 대신 현재 선택지만 표시한다. `CoreLoopController`는 카드 ID와 선택 ID를 독립 세션 또는 런 진행 세션에 전달하고, 실제 플레이에서는 다음 프레임에 입력 잠금을 해제한다. 선택 처리 중 표시 모델이 바뀌어도 현재 선택 목록의 스냅샷을 사용해 같은 GUI 이벤트에서 이전 인덱스를 다시 읽지 않는다.

`StageProgressionSession`은 진행 상태가 `InBattle`일 때만 카드 사용 시작과 선택 완료를 기존 `CoreLoopSession`에 전달하고, 승인된 입력 뒤 기존 전투 종료 동기화 경로를 한 번 호출한다. 카드 효과가 전투를 끝내면 기존 승리·패배와 지속 영혼 규칙을 그대로 사용하며, 끝내지 않으면 진행 상태는 `InBattle`로 유지한다.

CU-05 신규 8개를 포함해 관련 Presenter·진행 통합 28/28, CoreLoop 117/117, StageProgression 34/34와 전체 EditMode 151/151이 통과했다. `StageTest`에서 런을 시작해 `CoreLoopTest`의 같은 전투 인스턴스에 연결되는 것을 확인했다. Game View에서는 수정 구슬의 3개 선택, 해머 비용 선택, 리볼버의 1~10 선언, 보위 나이프 즉시 결과와 최근 결과·입력 잠금 해제를 확인했다. 양쪽 씬의 누락 스크립트·깨진 프리팹·기타 문제와 최종 Console Error/Warning은 0이었다. 즉시 모드 UI로 연결했으므로 씬 직렬화, 패키지와 외부 에셋은 변경하지 않았다.

### 2.6 CU-06 검증 결과

2026-07-20에 검증 전용 `CardUseSystemValidationTests` 5개를 추가했다. 독립 전투의 리볼버 승리와 재시작, 수정 구슬 후보 선택의 카드 소유권, 상대 비공개 숫자 1~10의 표시·결과 은닉, 카드 효과에 의한 런 승리·패배와 재시작을 각각 10회 반복했다. 매 반복에서 덱 가용 카드와 사용 중 카드 합계, 손패 ID 고유성, 새 전투 인스턴스, 지속 영혼과 사용 상태 초기화를 확인했다.

신규 5/5, CoreLoop 122/122, StageProgression 34/34, 전체 EditMode 156/156이 통과했다. 실제 `StageProgressionController`·`StageProgressionRuntime`·`CoreLoopController` 경로에서도 보위 나이프 최종 승리와 수정 구슬 버스트 패배를 만들고, `StageTest` 결과 화면의 재시작을 거쳐 `CoreLoopTest`에 새 전투·초기 영혼·`Available` 카드가 생성되는 것을 확인했다. `CoreLoopTest`와 `StageTest`는 각각 문제 0, 최종 Console Error/Warning 0이었다. 런타임·씬·패키지·외부 에셋은 변경하지 않았다.

### 2.7 위협용 해머 현행 규칙 이관 결과

2026-07-22에 선택 종류를 `DiscardOpponentFaceUpCard`로 변경하고, 효과 선택지는 행위자의 상대 손패에 있는 공개 카드만 참조하도록 이관했다. 선택된 카드는 상대 손패에서 상대 버린 더미로 이동하고 해머 원본은 손에 남아 `Used`가 된다. 상대가 스탠드 상태면 시작 검증에서 비공개 카드 1장과 교체 드로우 가능 여부를 먼저 확인한 뒤 스탠드 취소, 기존 비공개 카드 비공개 폐기, 새 비공개 카드 지급을 수행한다.

적이 해머를 사용할 때도 같은 상대 대상 경계를 사용한다. 집행관과 최종 보스의 선택 단계는 플레이어 공개 카드 중 숫자가 가장 큰 카드를 우선하는 프로토타입 정책을 적용한다. `GameScenePresentation`은 해머를 상대 대상 효과로 분류하고 설명을 `적 공개 카드 1장 제거; 스탠드면 비공개 교체`로 표시한다. 씬 에셋·패키지·외부 에셋은 변경하지 않았으며 Unity 전체 EditMode 308/308을 통과했다.

### 2.8 보위 나이프·중간 버스트 현행 규칙 이관 결과

2026-07-22에 `CoreLoopBattle`의 플레이어·적 히트와 수정 구슬·위협용 해머·보위 나이프 효과가 라운드 진행 중 `VisibleHandValue`만 검사하도록 통일했다. 중간 버스트는 `RoundResolver.ResolveNumericBust`에 버스트 대상을 명시해 처리하며, 양쪽 스탠드 뒤의 `RoundResolver.Resolve`만 전체 손패를 비교한다.

`MilitaryKnifeEffectHandler`의 유지 정책 인터페이스와 기본 유지 구현을 제거했다. 강제 공개 드로우로 상대 공개 합이 21을 초과하면 즉시 숫자 버스트를 확정하고, 초과하지 않으면 방금 뽑은 카드 ID를 상대 버린 더미로 반드시 이동한다. 같은 처리기는 플레이어와 적이 사용하므로 양쪽에 동일한 규칙이 적용된다. `GameScene`·패키지·외부 에셋은 변경하지 않았다.

### 2.9 CU-M09 GameScene 해머 대상 직접 선택

`GameScenePresenter`는 보류 효과가 `DiscardOpponentFaceUpCard`일 때 선택 옵션의 `CardId`를 적 카드에 대응시켜 `GameSceneCardViewModel.CardEffectChoiceOptionId`로 투영한다. 실제 옵션에 포함된 상대 공개 카드만 ID를 가지며, 뒷면 비공개 카드·앞면 공개된 비공개 역할 카드·그 밖의 비대상 카드는 `null`을 유지한다.

`CardView`는 이 nullable ID를 표시 모델에서 보존한다. `GameManager.Update`는 클릭한 전투 카드에 옵션 ID가 있으면 일반 플레이어 카드 사용보다 먼저 `TryResolvePlayerCardChoice(optionId)`를 호출한다. ID가 없는 카드 클릭은 기존 흐름만 검사하므로 해머 대기 상태를 변경하지 않으며, 규칙 계층도 선택 ID를 다시 검증한다.

`GameSceneCombatHudPresenter`는 해머 직접 선택 대기에서 기존 안내 문구와 옵션 모드를 유지하되 `OptionActions`를 비운다. 리볼버·수정 구슬 등 다른 수동 선택형 효과는 기존 버튼 목록을 유지한다. 기존 `UpdateHover → CardView.SetHovered` 경로를 재사용하며 씬·프리팹·머티리얼은 수정하지 않는다.
...
11. 카드의 앞·뒷면 상태와 공개·비공개 역할은 별도 불변식으로 관리한다.
...
| `auto-pistol-7` | 리볼버 | 7 | Manual | AutoPistol |
| `auto-pistol-8` | 리볼버 | 8 | Manual | AutoPistol |
| `military-knife-9` | 보위 나이프 | 9 | Manual | MilitaryKnife |
| `military-knife-10` | 보위 나이프 | 10 | Manual | MilitaryKnife |
...
`BlackjackHand`가 라운드 배분·체인지·교체로 받은 비공개 역할 카드 ID를 보유한다. `BlackjackCard.IsFaceUp`은 숫자를 화면과 관측에 공개할지 나타낼 뿐 역할을 바꾸지 않는다.

```text
BlackjackHand
- HiddenCardCount
- IsHiddenCard(cardId)
- TryGetSingleHiddenCard(out card)
- GetPublicCards()
```

`GetPublicCards()`는 앞면이며 비공개 역할이 아닌 카드만 반환한다. 공개 합, 중간 버스트, 해머·화염방사기·회중시계·바알제붑·벨리알 등 `공개 카드` 대상 효과는 이 목록을 사용한다. 체인지, 해머의 비공개 교체, 리볼버, 거짓말 탐지기와 최종 승부는 `TryGetSingleHiddenCard`로 역할 카드를 사용한다.

`EnemyObservation.KnownPlayerHiddenCardRank`는 비공개 역할 카드가 실제 앞면일 때만 값을 가진다. 미공개 상태에는 카드 객체·ID·숫자를 전달하지 않는다. `EnemyPolicyDecisionSelector.TrySelectCertainAutoPistol`은 합법적인 리볼버 시작 후보 또는 보류 중 정확한 숫자 선택지를 일반 정책보다 먼저 선택한다.
...
1. 승인 후 사용 카드 공개
2. 버릴 수 있는 상대 공개 카드 ID 옵션 생성
3. 선택 카드를 상대 손에서 제거해 상대 덱 버린 더미로 이동
4. 상대 비스탠드면 완료
5. 상대 스탠드면 취소 후 비공개 카드 원자적 교체
6. 상대 숫자 버스트 확인

해머 원본은 별도 비용으로 버리지 않고 행위자 손에 `Used`로 남는다. 상대 스탠드 상태에서 교체 조건을 만족하지 않으면 시작 검증에서 거절한다.
...
3. 상대 공개 합 버스트면 `ResolveNumericBust`로 즉시 라운드 종료
4. 버스트가 아니면 방금 뽑은 카드만 상대 덱 버린 더미로 반드시 이동
5. 효과 완료 후 기존 상대 차례 흐름 실행

유지·버림 정책은 존재하지 않는다. 비공개 카드 숫자는 이 중간 버스트 판정에 포함하지 않으며 양쪽 스탠드 뒤 최종 `Resolve`에서만 포함한다.

## 10. 라운드 결과 확장

리볼버 등 직접 버스트를 숫자 초과와 구분해 기록할 수 있어야 한다.
...
`CoreLoopViewModel`에는 플레이어 카드 목록, 현재 효과 안내, 선택지와 최근 효과 결과를 추가한다. 상대 카드 ViewModel은 뒷면인 비공개 카드의 정의 키·숫자를 노출하지 않으며, 그 카드가 앞면 공개되면 숫자를 표시하되 비공개 역할과 왼쪽 위치는 유지한다.
...
- 해머가 상대 공개 카드만 대상으로 제시하고 자신의 카드 비용을 요구하지 않는다.
- 상대 공개 카드가 없으면 해머 사용을 상태 변경 없이 거절한다.
- 상대 비스탠드 시 선택한 공개 카드만 상대 버린 더미로 이동한다.
- 상대 스탠드 시 취소·비공개 교체·카드 소유권이 정확하다.
- 해머 교체 뒤 중간 버스트는 상대 공개 합으로만 판정한다.
- 나이프 조건 `공개 합 <= 16`과 덱 잔량을 검사한다.
- 나이프 강제 히트의 공개 합 버스트와 비버스트 강제 폐기가 맞다.
- 비공개 카드 때문에 전체 합만 21을 넘은 경우 중간 버스트하지 않고, 양쪽 스탠드 뒤 최종 승부에서 버스트한다.
...
### 13.7 CU-M04 — 비공개 역할과 공개 지식

- 비공개 카드 사용 뒤 `IsFaceUp == true`여도 `HiddenCardCount == 1`과 역할 ID가 유지된다.
- 앞면 공개된 비공개 역할 카드는 공개 합·중간 버스트·공개 카드 대상에서 제외된다.
- 전체 합과 최종 승부에는 해당 숫자가 포함된다.
- 적 관측은 미공개 숫자를 받지 않고, 공개된 비공개 숫자만 확정값과 100% 추론으로 받는다.
- 사용 가능한 리볼버가 있는 적은 성향과 무관하게 리볼버를 우선하고 정확한 숫자를 선언한다.
- 양측의 앞면 공개된 비공개 역할 카드도 GameScene의 플레이어 화면 왼쪽 위치를 유지한다.

### 13.8 CU-M05 — 적 공개 카드 호버 정보

- 공개된 적 수동·자동 효과 카드에는 숫자·이름·효과 설명과 `ShowHoverBadgeWhenUnavailable == true`가 투영된다.
- 아직 뒷면인 적 비공개 카드는 순위·이름·설명과 호버 배지 허용값을 노출하지 않는다.
- 비공개 역할 카드가 앞면 공개되면 역할·왼쪽 위치를 유지한 채 호버 정보만 공개된다.
- `CardView`는 공개된 사용 불가 적 카드의 배지를 표시하고 미공개 카드의 배지 문자열은 빈 값으로 유지한다.

### 13.9 CU-M06 — GameScene 리볼버 연출 상태
...
- `LeviathanDemonContractHandler`는 첫 리볼버 성공을 완료로 처리하고 실패에만 추가 발동을 요청한다. 첫 성공의 버스트가 다른 계약으로 방지되어도 재예측하지 않는다.
- 첫 실패 뒤 `CoreLoopBattle.HasPendingLeviathanAutoPistolRetry`가 같은 원본 카드 ID의 두 번째 `AutoPistol` 선택 대기를 안전하게 식별한다.
- 두 번째 성공은 영혼 비용 0, 두 번째 실패는 영혼 비용 1이다. 비용으로 영혼이 0이 되면 기존 즉시 패배 경계를 재사용한다.
- `GameSceneRevolverAnimationPhase`는 `Ready`, `ResolvedWithRetry`, `Resolved`를 사용한다. `ResolvedWithRetry`는 첫 실패 결과를 표시한 뒤 같은 리볼버를 준비 상태로 되돌려 새 예측을 받는다.
...
### 13.11 CU-M09 — 해머 대상 카드 직접 선택

- 해머 선택 대기에서 실제 옵션의 카드 ID와 옵션 ID가 상대 공개 카드 모델에 일대일로 투영된다.
- 뒷면 비공개 카드와 앞면 공개된 비공개 역할 카드는 옵션 ID를 받지 않는다.
- `CardView.Bind`가 `CanUse`와 독립적으로 대상 옵션 ID를 보존한다.
...
1. 공개 리볼버 사용 → 리볼버 준비 등장 → 숫자 선택 → 성공·실패 사격 결과
2. 비공개 카드 사용 → 즉시 공개 → 선택 중 다른 입력 잠금
3. 수정 구슬 사용 → 화면 하단에 덱 위 카드 2장 표시 → 카드 1장 직접 클릭 또는 `선택하지 않기`
4. 해머 사용 → 선택 버튼 미표시 → 상대 공개 카드 호버 확대·외곽선·배지 → 카드 직접 클릭 → 제거와 스탠드 취소·비공개 교체
5. 나이프 강제 히트와 버스트
6. 사용 완료 카드의 재사용 방지
...
- 아직 앞면 공개되지 않은 상대 비공개 값은 규칙 계층 밖으로 나오지 않는다. 앞면 공개된 비공개 숫자는 공개 정보로만 전달한다.
...
| 2026-07-29 | 이천서 | CU-M04 비공개 역할 ID와 앞·뒷면 상태 분리, 공개 합·효과 대상·최종 합 경계, 확정 공개 숫자 관측과 적 리볼버 우선 선택, 양측 왼쪽 표시 및 신규 5/5·CoreLoop 430/430 검증 반영 |
...
| 2026-07-19 | 이천서 | CU-03 리볼버 단일 비공개 카드 추측·성공/실패·정보 은닉 구현과 전체 EditMode 125/125 검증 반영 |
| 2026-07-19 | 이천서 | CU-04 수정 구슬 순서 보존·해머 단일 비공개 교체·나이프 강제 드로우와 정책 경계 구현, 전체 EditMode 143/143 검증 반영 |
| 2026-07-19 | 이천서 | CU-05 카드별 사용 표시·효과 선택 UI·독립 및 런 세션 전달·종료 동기화 구현과 전체 EditMode 151/151·Game View·씬·Console 검증 반영 |
| 2026-07-20 | 이천서 | CU-06 반복 회귀 5개, CoreLoop 122/122·StageProgression 34/34·전체 EditMode 156/156과 실제 런 승리·패배·재시작 검증을 반영해 1차 범위 마감 |
