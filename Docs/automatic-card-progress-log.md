...
> 버전: v1.0
> 현재 단계: 자동 카드 원본 공개 손패 유지 규칙 이관 완료
> 다음 단계: GameScene 수동 화면 검증 및 상점 자동 카드 편입
> 최종 갱신: 2026-08-01

> **원본 수명 정정 기록(2026-08-01)**
> 독극물·거짓말 탐지기·화염 방사기와 비재배분 부활초는 효과 완료 뒤 원본을 버리지 않고 공개 손패에 유지하도록 수정했다. 유지된 숫자는 공개 합과 버스트·최종 승부에 계속 반영한다. 부활초 소유자가 영혼을 지불한 전체 손패 재배분과 회중시계의 선택 폐기는 명시적 예외로 유지한다. 관련 회귀 43/43은 통과했다. 전체 EditMode는 867/869가 통과했으며, 이번 변경과 무관한 `DX02_U02` 프리팹 위치값과 `MOO01_U04` 색상 비교 2건이 단독 재실행에서도 실패했다.
