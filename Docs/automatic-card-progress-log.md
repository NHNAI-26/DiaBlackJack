...
> 버전: v0.9
> 현재 단계: 기존 AC-RV02 완료 · 독극물·부활초 개정 미구현
> 다음 단계: 개정 효과 코드·테스트 이관
> 최종 갱신: 2026-07-30
...
> 독극물은 영혼 담보·승리 시 5 회복 문구로 개정됐다. 부활초는 양쪽이 각각 영혼 1개를 버릴 수 있고, 실제 지불한 참가자만 자기 손패를 모두 버린 뒤 비공개 1장과 공개 1장을 다시 받는다. 한쪽의 거절은 다른 쪽 선택에 영향을 주지 않는다. 기존 양측 강제 손실·전체 라운드 재시작 구현과 테스트는 변경 전 증거이며 이번 문서 작업에서 코드는 수정하지 않았다.
...
| AC-RV03 | 이천서(AI 문서 정정 보조) | 미착수 | 부활초 양쪽 독립 선택·지불 참가자 손패만 재배분, 독극물 개정 코드 이관 |
