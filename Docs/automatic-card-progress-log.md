...
> 작업 식별자: AC-00~AC-06  
> 버전: v0.1  
> 현재 단계: AC-03 거짓말 탐지기와 정보 경계 완료
> 다음 단계: AC-04 화염 방사기와 회중시계
> 최종 갱신: 2026-07-25
...
| 자동 카드 정의·실행 | 5종 정의 완료, 독극물·거짓말 탐지기 실제 처리기 구현 완료·나머지 3종 미구현 |
...
- 소유자→상대 순차 공개 카드 폐기 또는 건너뛰기
- 스탠드 참가자·후보 없음 자동 건너뛰기와 원본 제외
- `PocketWatchEffectHandler`
- 사용 완료 수동 카드 재활성화와 자동 카드·자기 자신 제외
- 회중시계 원본 유지·폐기 선택과 같은 손 재발동 방지
- 플레이어·적 대칭 선택, 카드 위치·사용 상태와 대상·전체 회귀

UI, 진행 세션, 적 자동 선택 정책, 보상 풀과 적 프로필은 AC-04에서 구현하지 않는다.
