...
DC-05는 두 기존 UI 스크립트만 확장해 씬 직렬화 변경을 피했다. 벨페고르의 덱 위 숫자는 소유자 전용 문구로만 표시하고 상대 비공개 카드 숫자는 기존처럼 0 또는 `?`로 투영한다. 다음 작업은 화면에 비활성화된 사탄을 실제 구현하기 전 DC-D05·D06 규칙을 확정하는 것이다.
