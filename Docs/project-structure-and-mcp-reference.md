...
| `Assets/01. Scripts/Runtime/CoreLoop` | 전투 규칙·상태·세션, 전투 행동과 카드 정의·사용·효과 처리 기반 | CU-04 플레이어 일반 수동 카드 4종 처리기 제공 |
...
| `Assets/01. Scripts/Runtime/StageProgression` | 런·스테이지 순수 상태, 전투 연결과 전투 보상 데이터·런 덱·선택 상태 | RW-03 실제 승리 보상 생성·등급 주입·다음 전투 덱 전달 |
...
| `Assets/06.Packages/Tests/EditMode/CoreLoop` | 코어 루프·전투 행동·카드 정의·효과 처리 규칙·표시·Controller·반복 회귀 테스트 | CU-06 기준 122개 |
| `Assets/06.Packages/Tests/EditMode/StageProgression` | 진행 상태·전투·표시·전투 행동·런 카드 사용·전투 보상 기반/상태/세션/표시/반복 회귀 테스트 | RW-05 기준 65개 |
