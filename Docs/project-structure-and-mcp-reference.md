...
| `Assets/06.Packages/Tests/EditMode/StageProgression` | 진행·보상·상대 후보·선택 상태·표시·프로필 전투 변환·호환 테스트 | EUI-03 기준 117개 |
...
EUI-04는 기존 `EnemyInferenceDisplayModel`과 `FinalBossEnemyPolicy.CurrentDisplay`를 수정하지 않고 재사용했다. 프로필 없는 독립 전투와 알 수 없는 프로필 키는 추정 프로필을 만들지 않으며, 표시를 읽는 행위가 적 정책 상태를 바꾸지 않는다. 정식 아트 UI와 전체 선택·보상·보스·재시작 반복 마감은 EUI-05 이후 별도 범위다.
