...
이 절은 당시 DC-06 구현 증거다. 현행 구조에서는 DC-R02가 별도 권능 정의·`SatanPowerEffectHandler`·임시 권능 카드 경로를 제거하고 활성 계약 자체의 양면 행동으로 대체했으며, 최신 경계는 7.47을 따른다.
...
### 7.40 자동 발동 카드 AC-04 화염 방사기·회중시계

| 경계 | 현행 구조 |
| --- | --- |
| 처리기 등록 | `AutomaticCardEffectResolver` 기본 구성에 `FlamethrowerEffectHandler`와 `PocketWatchEffectHandler` 추가 |
| 화염 방사기 단계 | 기존 보류 상호작용을 소유자·상대 순으로 새로 발급하고 첫 카드 이동 뒤 상대 후보 재계산 |
| 폐기 검증 | `AutomaticCardEffectContext`가 공개 여부·선택 주체 손패·원본 제외를 적용 순간 재검증한 뒤 실제 소유자 덱에 폐기 |
| 회중시계 후보 | 소유자 공개 손패의 `Manual + Used`만 제공하며 자동 카드·원본·`Available` 제외 |
| 재활성화 | `BlackjackCard.TryReactivate`가 `Used → Available` 상태만 바꾸고 카드 효과는 실행하지 않음 |
| 원본 위치 | 별도 `PocketWatchSourceDisposition`에서 `Discard` 또는 `RetainFaceUp`; 유지 시 `Unavailable`을 보존해 같은 손 비재발동 |
| Unity 검증 | 대상 11/11·CoreLoop 316/316·전체 EditMode 445/445·컴파일 오류 0 |
| 변경 보호 | `GameScene`·UI·세션·보상·적 자동 정책·프리팹·Packages·외부 에셋 무변경 |

AC-04도 `mcpforunity://custom-tools`, 인스턴스, 편집기 상태와 프로젝트 정보를 확인한 뒤 같은 Unity 6000.3.10f1 편집기에서 검증했다. 신규 파일을 포함한 강제 에셋 갱신 뒤 컴파일과 세 범위 테스트를 수행했고, 활성 `GameScene`은 열린 상태만 확인했으며 직렬화하지 않았다.

### 7.41 자동 발동 카드 AC-05 부활초

| 경계 | 현행 구조 |
| --- | --- |
| 처리기 등록 | `AutomaticCardEffectResolver` 기본 구성에 `ResurrectionHerbEffectHandler` 추가 |
| 선택 조건 | 거절은 항상 제공하고 양측 현재 영혼이 모두 2 이상일 때만 재시작 제공·해결 시 재검증 |
| 전이 모델 | `RoundTransition`이 부활초 원인·이전/새 라운드·원본 물리 ID·소유자를 기록하고 승자/피해 없음·연속 처리 취소를 명시 |
| 라운드 정리 | 양측 영혼 1, 독극물 예약·탐지기 지식·계약 라운드 훅·양측 손패와 스탠드를 정리한 뒤 `StartRound` 한 번 |
| 부모 취소 | `RestartRound` 완료 흐름이 수정 구슬·보위 나이프 부모 효과와 남은 차례 재개를 취소 |
| 결과 분리 | 최근 실제 `LastResolution`은 덮어쓰지 않고 `LastRoundTransition`으로 특수 재시작을 별도 노출 |
| Unity 검증 | 대상 11/11·CoreLoop 327/327·전체 EditMode 456/456·컴파일 오류 0 |
| 변경 보호 | `GameScene`·UI·세션·보상·적 자동 정책·프리팹·Packages·외부 에셋 무변경 |
...
### 7.43 게임 저장·이어하기 SV-00~SV-02 구조 대조
...
| SV-02 DTO | `RunSaveEnvelope` 필드 계층이 순수 스냅샷을 Unity `JsonUtility` 형식과 분리하고 체크포인트·상태·무늬를 안정 문자열로 기록 |
| SV-02 파일 경계 | `IRunSaveFileStore`가 실패 주입을 허용하고 `SystemRunSaveFileStore`만 영구 데이터 경로와 .NET 파일 API를 사용하며 단일 파일명만 허용 |
| SV-02 원자성 | 임시 기록→재로드·역직렬화·도메인 검증→기본을 백업→임시를 기본으로 교체, 마지막 실패 시 이전 기본 파일 복원 |
| SV-02 불러오기 | 기본 손상 시 유효 백업 사용, 양쪽 손상·미지원 버전·콘텐츠 불일치·터미널·빈 레거시를 명시적 결과로 구분 |
| SV-02 보호 | 레거시 `SaveLoadSystem`·씬·프리팹·Packages·HONG RF/Shop·Shim0Hwan 아트 무변경, 새 패키지·외부 에셋 없음 |
...
SV-02는 Unity MCP 최종 대상 `a77adba995914df8ba88ca9a89f4a137` 8/8,
StageProgression `b76f39b70e80469eae011991e162d887` 149/149, 최종 전체
`0bd47501216943e7b2ef4e8880720bd1` 491/491을 통과했다. 최초 컴파일 오류 1건은
지역 변수 초기화로 수정한 뒤 재컴파일해 오류 0건을 확인했다. 최종 Console에는
...
### 7.46 게임 저장·이어하기 SV-05 Runtime·UI 검증
...
| 화면 검증 | `StageTest` 1280×720 런 메뉴, 1920×1080 시작 악마·상대 선택과 선택 뒤 저장 성공 표시 확인 |
| 후속 경계 | 실제 운영체제 프로세스 종료·재실행과 RF 상점·사건 완료 체크포인트는 SV-06 |
| 변경 보호 | `GameScene`·씬·프리팹·Packages·HONG RF/Shop·Shim0Hwan 아트·외부 에셋·오픈소스 무변경 |

Unity MCP 활성 인스턴스 `DiaBlackJack@5635a4cdcfecc8dd`, Unity 6000.3.10f1에서
전용 job `ff6eba2778bc492da8beab20a70d09f7` 9/9, StageProgression job
`8ada54ae408f4490a34e518d685ee5aa` 189/189, CoreLoop job
`b2b79779112845118ff3de750378f7ca` 362/362, 전체 job
`d6ad875f5ed5423197e8d77710570f30` 551/551를 확인했다. `StageTest.unity`는
dirty 상태가 아니었고, 화면 검증 중 만든 영구 경로 예약·저장 파일은 종료 뒤 제거했다.

### 7.47 악마 계약 DC-R02 사탄 활성 계약 양면 검증

| 경계 | 확인 결과 |
| --- | --- |
| 상태 | `SatanRuntimeState`가 계약별 종말 카운트 4·현재 `SatanContractFace`·대가 적용 여부·소유자 정상 차례 진행을 보존 |
| 차례 훅 | `DemonContractResolver`가 소유자 방향 정상 차례 시작 감소와 종료 뒤집기를 분리하고 계약 선택 차례는 윗면 유지 |
| 대가·제한 | 0 도달 시 영혼 2를 한 번만 차감하고 계약은 전투 종료까지 유지, 소유자의 스탠드와 모든 버스트 원인 방지 |
| 활성 계약 행동 | `CoreLoopBattle`이 활성 계약 물리 ID를 재검증해 윗면 서로 다른 두 숫자 선언 또는 아랫면 상대 강제 공개 히트를 실행 |
| 자동 연속 처리 | `AutomaticCardContinuationKind.DemonContract`가 강제 히트의 자동 카드 선택 뒤 계약 처리를 한 번만 재개하고 중복 차례 완료 방지 |
| 카드 카탈로그 | 별도 숫자 8/10 사탄 권능 정의·`SatanPowerEffectHandler` 제거, enum 숫자 호환용 `LegacySatanPower` 슬롯만 비실행 상태로 유지 |
| 적 AI·정보 | 적 후보/결정에 활성 계약 출처 ID와 면을 보존하고 광신도는 공개 `EnemyNumberInference`로만 숫자 선언, 실제 비공개 숫자 비노출 |
| 세션·표시 | CoreLoop/StageProgression 세션이 활성 사탄 입력을 전달하고 표시 모델이 `종말 카운트 N · 윗면/아랫면`과 대가 적용 여부를 투영 |
| 검증 | 전용 12/12·CoreLoop 363/363·전체 EditMode 553/553, 실패·건너뜀 0 |
| 변경 보호 | `GameScene`·씬·프리팹·Packages·HONG RF/Shop·Shim0Hwan 아트·외부 에셋·오픈소스 무변경 |
| 후속 경계 | 실제 월드 활성 계약 카드 클릭·호버·면 뒤집기 연출과 두 해상도 화면은 DC-R05 |

Unity MCP에서 최종 전용 job `a8941795cdc7467b9fd0f1556ec9980b` 12/12, CoreLoop job
`503571cfabf5471c924dc64a79a345d4` 363/363, 전체 job
`68bd6a58eea7459e8a1df78391e6902a` 553/553을 확인했다. 최종 콘솔 오류 분류 2건은 두
...
> 이 절은 2026-07-28 당시 DC-R03 검증 기록이다. 현행 레비아탄 판정은 7.53 CU-M07을 따른다.

| 경계 | 확인 결과 |
| --- | --- |
| 마몬 상태 | `MammonRuntimeState`가 계약별 현재 주사위와 현재 소유자 차례의 유지·재굴림 선택 가능 여부를 보존 |
| 마몬 행동 | 매 소유자 차례 시작에 `MammonReroll` 선택을 열고, 유지는 정상 행동으로 복귀하며 재굴림은 주사위 갱신과 버스트/상대 차례 전환을 한 행동으로 완료 |
...
| 카드 이동 | 최초 자동 최근 카드 선택은 후속 정정으로 폐기됨. 현재는 두 단계 상호작용에서 소유자가 고른 양측 공개 카드 ID를 `BattleParticipant.TryDiscardCard`로 각 원소유자 버린 더미에 이동 |
...
| 변경 보호 | `GameScene`·씬·프리팹·Packages·HONG RF/Shop·Shim0Hwan 아트·외부 에셋·오픈소스 무변경 |
| 후속 경계 | 이 표의 1차 검증 뒤 카드 선택 규칙이 정정됐으며 현재 구조는 아래 7.50 참조 |

### 7.50 악마 계약 DC-R04 2/5 선택·차례·공개 유입 경계 검증

| 경계 | 확인 결과 |
| --- | --- |
| 카탈로그·Resolver | 아스모데우스·아자젤을 추가해 현재 구현 여덟 종 제공, enum 값은 저장 호환을 위해 뒤에 추가 |
| 바알제붑 선택 | 소유자 영혼 1 지불 뒤 자신의 공개 카드·상대 공개 카드를 별도 상호작용 ID로 선택하고 완료 시 두 물리 ID를 재검증; 오래된 입력은 무변경 거절 |
| 아스모데우스 | 정상 차례 시작 뒤 상대 비스탠드·드로우 가능 조건에서 선택을 열며 선택 자체는 정상 행동을 보존, EP-R04부터 숫자 7 이하 수동 카드는 전용 불가 사유로 차단 |
| 아자젤 | 공개 카드 유입 이벤트가 동일 숫자를 감지해 자동 카드·숫자 버스트보다 먼저 계약 버스트 처리, 계약 즉시와 정상 히트 뒤 공개 수동 효과 카드를 왼쪽부터 한 번씩 실행 |
| 연속 처리 | `AzazelCardEffectSequence`가 시작 시 카드 ID·순서를 고정하고 선택형 효과 뒤 다음 카드로 재개하며, 아스모데우스 강제 히트와 아자젤 버스트는 기존 자동 카드 continuation·사탄 방지·바알제붑 대체 뒤 정확한 소유자 차례로 복귀 |
| 적 AI·정보 | 적 아스모데우스는 공개 선택으로 강제 히트를 택하고 적 아자젤도 같은 소유자 방향 규칙 사용; 숨은 숫자·덱 순서 비노출 |
| 검증 | 전용 11/11·여덟 종 400시드·100자동전투·CoreLoop 390/390·바알제붑 정정 8/8·전체 EditMode 579/579 통과; 최종 Console 6건은 Test Framework 안내이며 게임 코드 오류 0 |
| 변경 보호 | `GameScene`·씬·프리팹·Packages·HONG RF/Shop·Shim0Hwan 아트·외부 에셋·오픈소스 무변경 |
| 후속 경계 | DC-R04 3/5 파이몬·벨리알, 월드 계약 입력·두 해상도는 DC-R05 |

### 7.51 악마 계약 DC-R04 3/5 카드 수명·소유권 경계 검증

| 경계 | 확인 결과 |
| --- | --- |
| 순수 규칙 위치 | 파이몬·벨리알 처리기와 `DemonContractCardState`를 `CoreLoop/DemonContracts`에 두고 `UnityEngine` 참조 없이 유지 |
| 파이몬 | 상대 버스트 뒤 가능한 양쪽 덱 중 하나의 위 2장을 분리해 건너뛰기/추방을 처리하고, 남은 순서와 전투 종료 원소유권을 복구 |
| 파이몬 판정 | 선택 뒤 기존 결과의 임시 승자에게만 18 이하 대가를 한 번 적용하고 새 승자 쪽 파이몬 재연쇄를 막는 프로토타입 결정으로 고정 |
| 벨리알 | 계약 즉시 공개 카드 폐기, 정상 차례 시작 상대 공개 카드 탈취·즉시 사용·상대 스탠드 취소·정상 행동 보존, 새 라운드 드로우 전 영혼 1 적용 |
| 카드 ID·복구 | 양측 런 카드 ID가 겹쳐도 전투 전용 대리 ID로 충돌을 피하고 라운드 종료 현 소유자 버린 더미·전투 종료 원소유자 덱 복구를 보장 |
| 적 AI·정보 | 파이몬은 계약으로 확인한 2장만 비교하고 벨리알은 공개 카드만 비교해 기존 숨은 숫자·덱 순서 비노출 경계를 유지 |
| 검증 | 파이몬·벨리알 10/10(job `b5e5706ba044447ba4bd63c84f7ee364`)·광신도 9/9(job `fd969132ba944260af874e4f51e8c82e`)·CoreLoop 402/402(job `aa6c935ffe134a3ea8332b8256156849`)·전체 EditMode 591/591(job `56d59206ba42450a958d777be261f141`) 통과 |
| 변경 보호 | `GameScene`·씬·프리팹·Packages·HONG RF/Shop·Shim0Hwan 아트·외부 에셋·오픈소스 무변경 |
| 후속 경계 | DC-R04 4/5 바포메트, 월드 계약 입력·두 해상도는 DC-R05 |

### 7.52 카드 사용 CU-M04·적 AI EP-R02 정보 경계 검증

| 경계 | 확인 결과 |
| --- | --- |
| 순수 규칙 위치 | `CoreLoop/BlackjackHand`가 비공개 역할 카드 ID를 `BlackjackCard.IsFaceUp`과 별도로 보존하며 Unity 참조 없음 |
| 합계·대상 | 공개 합·중간 버스트·해머 등 공개 대상은 `GetPublicCards()`를 사용하고, 최종 승부·교체·리볼버는 비공개 역할을 유지 |
| 적 관측 | `EnemyObservationFactory`가 아직 뒷면인 숫자·카드 ID를 차단하고 앞면 공개된 숫자만 `KnownPlayerHiddenCardRank`로 복사 |
| 적 행동 | `EnemyPolicyDecisionSelector`가 모든 프로필 정책보다 먼저 사용 가능한 리볼버와 정확한 숫자 후보를 선택 |
| GameScene | 씬·프리팹을 수정하지 않고 `GameScenePresentation`에서 역할 기준 정렬·공개 여부 표시를 수행 |
| 검증 | CUM04 5/5·CoreLoop 430/430·StageProgression 189/189·전체 EditMode 619/619 통과 |
| 변경 보호 | 씬·프리팹·Packages·외부 에셋·오픈소스 무변경 |

### 7.53 CU-M07 레비아탄 조건부 재예측·GameScene 연출 검증

| 경계 | 확인 결과 |
| --- | --- |
| 순수 규칙 위치 | 레비아탄 추가 발동·영혼 대가 불변식은 `CoreLoop/DemonContracts/LeviathanDemonContractHandler.cs`와 `CoreLoopBattle.cs`에 두고 Unity 참조 없이 유지 |
| 연속 처리 | 첫 성공은 즉시 종료, 첫 실패만 동일 원본 카드 컨텍스트의 새 선언을 열며 두 번째 성공은 비용 0·두 번째 실패는 영혼 비용 1 |
| 표시 모델 | `GameSceneRevolverAnimationPhase`의 `Ready/ResolvedWithRetry/Resolved`가 첫 실패 결과와 두 번째 선택 대기를 분리 |
| GameScene | `GameManager`가 첫 실패 연출 뒤 기존 리볼버 루트를 준비 상태로 되돌리고 최종 판정에 일반 성공·실패 Animator 트리거 사용 |
| 검증 | 신규 4/4·관련 클래스 18/18(job `35bd1df8d35e467d9fdbaaf1359027f2`)·CoreLoop 442/442(job `0c51483524f74466995c4f33b6b15f3d`)·전체 EditMode 631/631(job `4546b3326ead476590aa8bfa422cd3e2`) 통과 |
| 변경 보호 | 씬·프리팹·Animator Controller·Packages·외부 에셋·오픈소스 무변경 |

### 7.54 GSV-01 적 프로필 스프라이트·독립 GameScene 전투 구성 연결
...
### 7.56 RF 카드 성장 상점 통합 문서 개정

| 경계 | 현행 결정과 구현 상태 |
| --- | --- |
| 정식 순서 | 일반전 승리→프로필별 골드 1회→상점, 두 번째 상점→보스, 보스 승리→골드 결과→`RunVictory` |
| 보상 상태 | 기존 `RewardSelection`, `BattleRewardCatalog`과 RW 테스트는 유산 기능으로 보존하되 정식 RF 세션·화면에서는 사용하지 않음 |
...
### 7.58 RF-05 반복 회귀·씬 정적 검증

| 경계 | 확인 결과 |
| --- | --- |
| 반복 규칙 | 전체 승리·재시작 10회, 상점 네 행동 10회, 오래된 입력 10회, 세 전투 위치 패배·재시작 각 10회 |
| 상대·골드 | 네 일반 프로필을 실제 후보에서 선택하고 3·4·6·7골드 정산 확인 |
| 씬 | `StageTest`·`GameScene` 활성 Build Settings, Runtime 전투 목적지, GameManager·ShopController, 누락 스크립트 0 |
| 대상 실행 | RF-01~05 Unity 비의존 49/49 |
| Unity 실행 | 원본 Editor 유지, `Temp/RF05UnityClone` 격리 복제본 Unity 6000.3.10f1 전체 EditMode 744/744 |
| 오류 | 실패 0·건너뜀 0·미결정 0, `error CS`·컴파일 실패·처리되지 않은 예외 0 |
| 라이브 한계 | Unity MCP 8080 initialize handshake 실패로 `StageTest ↔ GameScene` Play Mode와 720p·1080p 시각 검증 미실행 |
| 변경 보호 | 외부 에셋·외부 코드·오픈소스·Packages·새 의존성 무변경 |

### 7.59 RFM02 GameScene 직접 전투·진행 전투 분리

| 항목 | 현재 구조 |
| --- | --- |
| 새 런 생성 | GameScene 직접 실행은 새 런 Runtime을 생성하지 않는다. StageTest는 기존 `CreatePrototypeSession`을 유지한다. |
...
| GameScene 직접 실행 | 활성 진행 전투가 없으면 기존 독립 `CoreLoopSession`을 즉시 생성해 월드 전투 화면을 시작한다. 진행 View·Controller·선택 메뉴를 만들지 않는다. |
| StageTest 직접 실행 | 씬 작성 Runtime과 `progressionSceneName=StageTest`를 유지하며 시작 악마 선택을 새로 강제하지 않는다. |
| 검증 | GameScene 직접 실행 시 진행 Controller 없음·GameManager Battle `PlayerTurn`, 진행 재진입 시 GameManager·Battle 유지, StageTest 분리, Console Error 0 |
...
| 날짜 | 작성자 | 변경 내용 |
| --- | --- | --- |
| 2026-07-30 | 이천서 | RFM02 책임을 정정해 GameScene 직접 실행은 기존 독립 전투를 즉시 시작하고, 활성 진행 전투가 있을 때만 해당 런 전투를 채택하도록 기록했다. StageTest 기존 Runtime은 유지한다. |
...
| 2026-07-28 | 이천서 | 바알제붑 양측 공개 카드 직접 선택 정정과 DC-R04 2/5 아스모데우스 차례 선택·6 이하 제한, 아자젤 재활성화·동일 숫자 선행 버스트의 순수 C# 이벤트·연속 처리·적 공개 관측 구조 및 바알제붑 8/8·신규 11/11·400시드·100자동전투·CoreLoop 390/390·전체 579/579 결과 추가; GameScene·씬·프리팹·Packages·외부 에셋 무변경 |
