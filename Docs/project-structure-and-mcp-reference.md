...
| 경로 | 역할 | 코어 루프 사용 여부 |
| --- | --- | --- |
| `Assets/00. Scenes` | 게임 및 테스트 씬 | 1~2단계 수정 없음, 3단계 예정 |
...
| `Assets/01. Scripts/Runtime/UI` | 공용 uGUI 구성 요소 | 1~2단계 제외, 3단계 예정 |
...
1~2단계 코어 루프는 새 런타임 어셈블리를 만들지 않고 `Border` 어셈블리 안의 `DiaBlackJack.CoreLoop` 네임스페이스에 배치했다. 기존 어셈블리 수를 늘리지 않으면서 UI와 독립된 순수 C# 규칙·상태 흐름을 유지하기 위한 결정이다.
...
| 스크립트 정적 검증 | 1단계 7개 및 2단계 변경 6개, 오류 0, 경고 0 |
| Unity 컴파일 | 성공 |
| Unity Console | 게임 컴파일 오류 0건. Test Framework 결과 저장 경로 메시지 1건이 `Exception` 유형으로 분류됨 |
| EditMode 테스트 | 전체 23개 통과, 실패 0 |

1단계 MCP 전체 EditMode 실행은 한 차례 30초 초기화 제한에 걸렸으나 대상 테스트 재실행은 15개 모두 통과했다. 2단계 착수 시 HTTP 서버가 일시적으로 응답하지 않았으나 Unity 도메인 리로드 후 같은 프로젝트 인스턴스로 연결이 복구되었고, 전체 EditMode 23개가 통과했다.
