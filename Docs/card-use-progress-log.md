...
### 2026-08-08 CU-M14 무기 애니메이션 시작 안정화

- 망치는 대상 카드의 현재 손패 레이아웃 위치를 확인하기 전 공격 trigger를 보내지 않는다. 위치가 아직 없으면 `Preparing`에서 재시도하고, 준비 중에도 전투 타임라인이 종료를 기다린다. 타격 직전 위치를 다시 읽으며 이전 공격 위치와 프리팹 기본 위치를 사용하지 않는다.
- 나이프는 타임라인에서 보류한 `Ready` cue를 먼저 실제 재생한 뒤 `Resolved` cue를 처리한다. 루트·Animator가 활성화되고 trigger가 적용된 뒤에만 cue를 소비한다. Animator 기본 상태는 실제 전체 경로 `Base Layer.Empty`로 교정했다.
- 망치 Animator의 base·ready·smash 상태도 전체 경로로 고정했다. 관련 무기·독극물·총잡이·프레젠테이션 회귀 87/87이 통과했다.
- 전체 EditMode는 1,224개 중 1,206개 통과했다. 실패 18개는 도감·상점·프롬프트·카드 프리팹·말풍선 등 기존 비관련 자산 기대값이며 CU-M14·독극물·총잡이 신규 테스트 실패는 없다. Play Mode 스모크는 기존 디버그 패널의 `ProcessInput` 반사 호출 인자 불일치로 수동 반복 재생 증거를 완료하지 못했다.
- 후속 수정은 망치의 `ReadyAttack → Smash`와 나이프의 시작·성공·실패를 trigger 전이 대신 검증된 전체 Animator 상태로 직접 재생한다. 숨겨진 상대 나이프 원본 경로도 `Ready → Resolved` cue를 유지한다. 독극물 담보로 영혼 0이 된 경우는 라운드 결과와 승리 회복 뒤까지 사망을 유예한다.
- 후속 독극물 회귀 16/16과 관련 무기·독극물·행동 해골·표정 회귀 66/66이 통과했다. 전체 EditMode는 1,228개 중 1,210개 통과했으며, 실패 18개는 앞서 기록한 도감·상점·프롬프트·카드 프리팹·말풍선 등 비관련 자산 기대값과 동일하다. Animator 프리팹의 망치 3개·나이프 6개 전체 상태 직접 진입도 모두 성공했다.
- 실화면 후속 제보에 따라 나이프 `Ready` 재생을 카드 공개·비교·자동 효과 분기보다 먼저 실행하도록 옮겼다. 공개 프레임이 보류되거나 중간 프레임이 건너뛰어져도 나이프가 결과 직전에 뒤늦게 등장하지 않는다.
- 망치의 애니메이션 대상 `Hammer_Rigging/Target`을 직접 이동하던 방식을 제거했다. Animator가 해당 Transform을 다시 쓰더라도 유지되는 비애니메이션 루트를 카드 위치만큼 이동하고, 연출 종료 시 원래 루트 위치를 복원한다.
- 후속 관련 회귀 76/76이 통과했다. 전체 EditMode는 1,229개 중 1,211개 통과했으며 기존 비관련 실패 18개는 동일하다. 실제 프리팹 샘플에서 나이프 등장 프레임과 망치 충돌음 프레임은 Renderer 활성, dissolve 0, 카메라 절두체 내부임을 확인했다.
- 나이프 결과 프레임에서 `ApplyView`가 성공·사라짐 상태를 카드 공개보다 먼저 시작하던 순서를 수정했다. 이제 `Ready 유지 → 비공개 카드 공개 완료 → 결과 상태 1회 시작 → 결과 View 적용` 순서를 사용한다. 관련 회귀 76/76, 전체 EditMode 1,229개 중 1,211개 통과이며 기존 비관련 실패 18개는 동일하다.

#### 권장 커밋 제목

`fix : 무기 연출 시작과 자동 턴 재개를 안정화하다`
...
### 2026-08-09 리볼버 원본 표시·나이프 카드 공개 완주 보완

- 리볼버 결과 cue는 행동 ordinal뿐 아니라 최신 공개 행동의 실제 카드 ID와도 일치해야 생성된다. 다른 리볼버를 사용할 때 이전 리볼버 결과나 강조 표시가 다시 선택되지 않는다.
- 나이프로 추가된 카드가 자동 발동 카드이면 뒤집기 중간의 앞면 교체 시점이 아니라 전체 flip 완료까지 전투 타임라인을 기다린다. 다음 자동 효과 snapshot이 같은 `CardView`를 재바인딩해 애니메이션을 끊지 않는다.
- 서로 다른 7·8 리볼버와 나이프 강제 자동 카드 회귀 3/3, 관련 프레젠테이션·자동 카드 fixture 74/74가 통과했다. 전체 EditMode는 1,265개 중 1,248개 통과했으며 실패 17개는 기존 도감·상점·말풍선·카드 프리팹 등 비관련 기준선이다. Unity 컴파일 오류는 0건이다.

### 2026-08-09 나이프 시퀀스 격리·리볼버 결과 우선순위 보완

- 나이프 결과는 같은 라운드·행동 ordinal·원본 카드·행위자의 준비 Loop가 확인된 경우에만 재생한다. 준비 상태가 유실되면 결과로 바로 점프하지 않고 새 generation으로 등장부터 복구한다.
- 공개·결과 pairing에도 행동 ordinal을 포함해 같은 카드가 한 라운드에서 다시 사용되어도 이전 공개 snapshot과 결합되지 않는다.
- 적 바알제붑이 플레이어 리볼버를 가로막는 동안에도 완료 결과를 플레이어의 pending 선택보다 우선해 `Resolved` cue로 만든다. GameManager는 해당 공격 결과 완료를 확인한 뒤에만 보류된 계약 처리를 재개한다.
- 관련 fixture 200/200, 전체 EditMode 1,250/1,267 통과다. 실패 17건은 기존 도감·상점·말풍선·카드 프리팹 기준선과 동일하다.

### 2026-08-09 나이프 중복 복구 차단

- 후속 실플레이에서 동일 `Resolved` snapshot이 다시 순회될 때 복구 경로가 새 `Ready`를 만들어 나이프가 여러 번 등장하는 회귀를 확인했다.
- 이미 소비한 결과와 같은 행동에서 한 번 복구를 시도한 결과는 다시 등장 복구하지 않는다. 정상 결과는 동일 준비 시퀀스에서 한 번만 재생하고 공통 종료 경로로 숨긴다.
