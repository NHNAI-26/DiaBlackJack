...
> 프로젝트: DiaBlackJack  
> 기록·구현 책임자: 이천서  
> 현재 단계: CU-M07 레비아탄 조건부 재예측·연출 연결 완료
...
### 6.8 CU-M09 — 해머 표정 제거·리볼버 피격 타이밍

- `ThreatHammer`를 공격 반응 분류에서 제외해 선택·처리 중 상대가 공격 위기 또는 공격받음 표정으로 바뀌지 않게 했다. 해머 카드 제거와 무기 애니메이션은 유지한다.
- 리볼버 성공 판정 직후 대상의 `Attacked`를 `AttackThreatened`로 보류하고, 플레이어·적 성공 클립의 기존 `gun-fire` VFX 프레임에 `NotifyShotImpact` 이벤트를 추가했다.
- 이벤트 수신 시 실제 대상만 `Attacked`로 렌더링한다. 실패, 레비아탄 재준비, 연출 숨김에서는 보류 타격 상태를 제거한다.
- Unity MCP 대상 회귀 6/6(job `3e4b9227478147158ddfe92036630e2b`), 스크립트·테스트 어셈블리 컴파일, 게임 코드 오류 0을 확인했다. Console 3건은 Test Framework 결과 파일 저장 안내다.
- 전체 EditMode는 766건 중 기존 광신도 자동전투의 `Enemy Beelzebub choices exceeded the resolution limit` 1건이 재현됐다. CU-M09 대상 6건은 모두 통과했으며 해당 실패는 이번 변경 파일과 무관하므로 별도 후속으로 남긴다.
- 기존 리볼버 애니메이션 에셋 2개만 이벤트 타이밍을 수정했다. 새 외부 에셋·오픈소스·패키지·씬·프리팹은 추가하거나 변경하지 않았다.

#### 권장 커밋 제목

`fix : 무기 피격 표정을 실제 타격 시점에 맞추다`
...
| 날짜 | 작성자 | 변경 |
| --- | --- | --- |
| 2026-07-30 | 이천서 | CU-M09 해머 상대 표정 반응 제거와 리볼버 총구 화염 프레임 피격 전환을 구현하고 대상 6/6·컴파일·게임 코드 오류 0을 확인했다. 전체 766건 중 기존 광신도 자동전투 실패 1건은 별도 기록 |
