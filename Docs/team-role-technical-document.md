...
| --- | --- | --- | --- |
| 이천서 | 프로젝트·기획 문서, 전투 코어·적 프로필·상대 선택과 전투 정보 UI 통합 담당 | 규칙·구조 결정, 적 5종 프로필·AI·카드 효과·전투 변환, 상대 선택·등급별 적 정보 UI, 테스트·완료 기록 관리 | 코어 루프·런 진행·전투 행동·카드 사용·전투 보상 완료, EP-06으로 적 프로필 1차 범위 완료, EUI-00 문서·담당 기준 확정(AI 보조) |
| Shim0Hwan(Git 식별자) | 아트·레벨 환경 담당 | FBX·텍스처·재질·셰이더, 전투 공간·조명·분위기와 아트 리소스 관리 | 아트 오브젝트·셰이더·`LevelDesign` 씬·시네머신 패키지 관련 커밋 확인 |
| HONG(Git 식별자) | 랜덤 이벤트·정식 런 진행 개발 예정 | 이벤트 생성·결과와 `전투→이벤트→전투→이벤트→보스` 정식 흐름 연결 | 폴더 구조·TMP 패키지 관련 커밋만 확인, 담당 게임 코드 미착수 |
...
코어 루프 1~4단계, 독립 런·스테이지 SP-00~SP-04, 전투 행동 BA-00~BA-05, 카드 사용 CU-00~CU-06과 전투 보상 RW-00~RW-05의 구현·검증·문서 마감이 완료되었다. 이천서는 EP-00부터 적 5종 전투 프로필·AI·카드 효과·실제 전투 변환을 담당하며 EP-01에서 프로필·미리보기·정책 기반과 결정적 전투 설정, EP-02에서 공개 관측·유효 행동·재검증과 적 카드 실행 경계, EP-03에서 공개 숫자 추론과 일반 적 3종 정책, EP-04에서 집행관 해머·나이프 후속 평가와 엘리트 표시·보상 연결, EP-05에서 최종 보스 3구간 정책·강행동 예고·런 승리, EP-06에서 선택 키의 실제 전투·보상 변환과 반복 상태 격리를 구현해 적 전투 프로필 1차 범위를 마감했다. EUI-00부터 상대 후보 생성·선택 UI와 전투 중 등급별 적 정보 UI도 이천서 담당으로 변경했으며, 현재는 문서·구현 기준만 완료되고 코드는 미착수다. 아트는 Shim0Hwan, 랜덤 이벤트와 정식 런 진행은 HONG의 계획 영역으로 유지한다.
...
| 예정 | 랜덤 이벤트·정식 런 진행 | 이벤트 후보·선택·결과와 `전투→이벤트→전투→이벤트→보스` 진행 연결 | 담당 경로·산출물은 착수 문서에서 확정 | 계획 역할이며 실제 구현 완료로 기록하지 않음 |
...
| EUI-01 | 후보 생성·선택 대기 상태 기반 | 이천서(AI 구현·검증 보조) | 대기 |
| EUI-02 | 후보 비교·지정·확정 화면 | 이천서(AI 구현·화면·검증 보조) | 대기 |
| EUI-03 | 선택 프로필의 실제 전투·보상 통합 | 이천서(AI 구현·검증 보조) | 대기 |
| EUI-04 | 일반·엘리트·보스 전투 정보 UI | 이천서(AI 구현·화면·검증 보조) | 대기 |
| EUI-05 | 반복 회귀·씬·화면·기록 마감 | 이천서(AI 구현·검증·기록 보조) | 대기 |
