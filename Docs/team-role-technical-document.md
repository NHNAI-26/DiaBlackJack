...
| --- | --- | --- | --- |
| 이천서 | 프로젝트·기획 문서, 전투 코어·적 프로필·상대 선택·전투 정보 UI·런 저장 통합 담당 | 규칙·구조 결정, Notion v0.7 적 6종 프로필·AI·카드 효과·전투 변환, 상대 선택·등급별 적 정보 UI, 저장·이어하기 Runtime/UI, 테스트·완료 기록 관리 | EP-R06과 RF-01A·RF-01B 골드 상태·승리 정산 완료; 정식 상점·런 UI는 RF-02 이후 협업(AI 보조) |
| Shim0Hwan(Git 식별자) | 아트·레벨 환경 담당 | FBX·텍스처·재질·셰이더, 전투 공간·조명·분위기와 아트 리소스 관리 | 아트 오브젝트·셰이더·`LevelDesign` 씬·시네머신 패키지 관련 커밋 확인 |
| HONG(Git 식별자) | 상점·정식 런 진행 후속 개발 예정 | 자동 카드 포함 RF-02 상점 거래와 `전투→골드→상점→전투→골드→상점→보스` 정식 흐름 연결 | RF-01A·RF-01B는 이천서가 선행 구현, RF-02~05 미착수 |
...
| RF-02 | 상점 구매·제거·휴식·나가기 | HONG | 미착수·착수 가능 |
