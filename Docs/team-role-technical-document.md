...
| --- | --- | --- | --- |
| 이천서 | 프로젝트·기획 문서, 전투 코어·적 프로필·상대 선택·전투 정보 UI·런 저장 통합 담당 | 규칙·구조 결정, Notion v0.7 적 6종 프로필·AI·카드 효과·전투 변환, 상대 선택·등급별 적 정보 UI, 저장·이어하기 Runtime/UI, 테스트·완료 기록 관리 | EP-R06과 RF-01~RF-04 도메인·진행 화면 코드 완료; RF-04 Unity 실화면 검증 대기(AI 보조) |
| Shim0Hwan(Git 식별자) | 아트·레벨 환경 담당 | FBX·텍스처·재질·셰이더, 전투 공간·조명·분위기와 아트 리소스 관리 | 아트 오브젝트·셰이더·`LevelDesign` 씬·시네머신 패키지 관련 커밋 확인 |
| HONG(Git 식별자) | 정식 런 진행 후속 개발 예정 | RF-05 반복 검증 협업 | RF-01~RF-04 코드는 이천서가 선행 구현, RF-05 미착수 |
...
| --- | --- | --- | --- | --- |
| 2026-07-30 | 정식 런 진입 RFM02 | StageTest 기존 테스트 흐름을 복원하고 GameScene 직접 실행에서만 시작 악마 2장 중 1장·상대 2명 중 1명 선택 뒤 같은 씬의 실제 전투를 시작하도록 재구성했다. 후속 실화면 재현에서 중복 카드 콘텐츠 부트스트랩이 새 GameManager 호스트를 삭제하는 수명 충돌도 수정했다. | `StageProgressionRuntime.cs`, `GameManager.cs`, `CardContentBootstrap.cs`, `FormalRunSystemValidationTests.cs`, 정식 런 4종·AI 활용 기록 | 이천서 기획·구현·검증 책임, AI 씬 책임·전투 미시작 원인 추적과 코드·테스트·문서화 보조; 대상 14/14·StageProgression 248/248, 재진입 3초 뒤 GameManager·Battle·PlayerTurn 생존, StageTest 분리·Console 0. 기존 CoreLoop 결함 2건 분리, 외부 에셋·패키지·씬·프리팹 변경 없음 |
...
| RF-04 | 진행 화면·씬 연결 | 이천서(AI 구현·검증 보조) | 코드 완료, RF04 6건 컴파일·비Unity 5건·누적 42/42; Unity 실화면 검증 대기 |
