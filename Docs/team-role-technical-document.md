...
## 2026-08-08 보완 작업 역할 기록

- 이천서: 증상 재현 조건, 튜토리얼 연출 순서, 나이프 버스트 비교 생략, 버튼 호버 중심 요구를 결정했다.
- AI: Unity MCP 게이트 확인, 관련 코드·직렬화 자산 분석, 구현, 회귀 테스트 추가, 관련/전체 EditMode 및 Console 검증을 담당했다.
- 구현 범위: 튜토리얼 화자·입장·패배·메인 메뉴 흐름, 나이프 상태 직렬화, 총잡이 상대 턴 실행 잠금, 리볼버 행동 ordinal, DefaultButton 중앙 pivot 동기화.
- 검증 상태: 신규 회귀 6건과 총잡이 라운드 경계 회귀 통과, 관련 묶음 198/199 통과, 전체 EditMode 1,228/1,246 통과, 컴파일 오류 0건. 범위 밖 기존 덱 프리뷰 HDR 색상 및 에셋·프리팹 기대값 테스트 18건은 별도 잔여 위험으로 기록한다.
...
| --- | --- | --- | --- | --- |
| 2026-08-08 | 전투 UI 피드백·사탄 연출 | 등장 중 합계 숨김 기준, 사탄 능력의 놀람·피격 표정과 공개 카드 표식 방향, 선택 UI 즉시 숨김, 호버 확대 중심, 일반 버튼 클릭음 범위를 결정하고 실제 화면 피드백으로 우측 하단 버튼 이동 회귀를 재제보했다. | `GameManager.cs`, `UIButtonScaleFeedback.cs`, `UISelectableSoundHook.cs`, `DefaultButton.prefab`, `SoundManager.prefab`, 사탄·HUD 관련 테스트 | 이천서 UI 기준·연출 순서 결정·실제 플레이 확인·최종 승인 책임, AI 경로 검토·코드·프리팹 연결·회귀 테스트·문서화 보조. 누적 관련 76/76, 컴파일 오류 0. 기존 `button_press.mp3`만 사용했으며 다른 팀원 기여는 확인되지 않아 배정하지 않음. |
| 2026-08-08 | 무기 연출·상대 턴·독극물 안정화 | 실제 플레이에서 망치·나이프 연출 누락과 순서 오류, 총잡이 중복 행동, 집행자 독극물 진행 정지와 강제 스탠드 표시 누락을 반복 재현·제보했다. 나이프의 기대 순서를 `등장 → 비공개 카드 공개 → 퇴장`으로 확정하고, 독극물 대가로 영혼이 0이 된 경우 라운드 종료까지 사망을 유예한다는 규칙을 결정했다. | `GameManager.cs`, `HammerAnimationController.cs`, `Hammer_Anim.prefab`, CoreLoop 독극물·상대 행동 처리, 행동 해골 표시, 관련 EditMode 테스트와 규칙·진행 문서 | 이천서 증상 재현·규칙 및 연출 순서 결정·최종 승인 책임, AI 코드 추적·구현·회귀 테스트·문서화 보조. 관련 76/76 통과, 전체 1,211/1,229 통과, 컴파일 오류 0. 잔여 18건은 범위 밖 기존 실패이며 장시간 실제 전투 시각 검증은 이천서 담당. 다른 팀원 기여는 확인되지 않아 배정하지 않음. |
...
- 이천서: 튜토리얼을 첫 플레이 자동 분기 대신 메인메뉴 버튼으로 여는 별도 씬으로 바꾸기로 결정, 우선 "본 게임은 무조건 정상 진행"부터 긴급 처리(문서 생략 지시)하고, 이어서 별도 씬 전환을 승인.
- **긴급 조치**: 첫 플레이 여부에 따라 자동으로 튜토리얼/일반 런을 갈라주던 로직을 없애고 본 게임이 항상 정상적인 일반 런으로 시작하도록 고정. 컴파일·테스트만 확인하고 짧게 보고(지시대로 문서 생략).
- **아키텍처 전환**: 이천서의 "따로 씬이 있는 게 낫나"라는 질문에 완전 새 GameScene 복제가 아니라 `StageTest`처럼 가벼운 부트스트랩 씬을 권장, 승인받아 진행.
  - `StageProgressionRuntime`에 튜토리얼 진입 여부를 나타내는 플래그를 추가해, 같은 컴포넌트를 `StageTest`(일반)와 신규 `TutorialScene`(튜토리얼) 양쪽에서 값만 다르게 재사용 — 전투/진행 화면 쪽 코드는 전혀 안 건드림.
  - `Assets/00. Scenes/TutorialScene.unity` 신규 생성(`StageTest.unity` 원본은 손대지 않음, `git status`로 확인).
  - 씬 전환 시 기존 싱글턴이 새로 만든 튜토리얼 전용 인스턴스를 밀어내 버리는 문제를 실제 플레이 테스트 전에 코드 검토만으로 미리 발견해 수정 — 튜토리얼 버튼을 눌러도 조용히 일반 런이 뜰 뻔한 버그를 사전 차단.
  - 메인메뉴에 "TUTORIAL" 버튼 추가, 클릭 시 튜토리얼 씬으로 전환.
- 변경 영역: `Bootstrap/StageProgressionRuntime.cs`, `UI/MainMenu/{MainMenuView,MainMenuController}.cs`, 신규 `00. Scenes/TutorialScene.unity`.
- 검증: 컴파일 0 오류, 전체 EditMode 1142개 중 1131개 통과(기존 베이스라인과 동일, 신규 회귀 없음). `git status` 확인 중 이 작업과 무관한 아트 에셋 2건(폰트 아틀라스, 카드 머티리얼)이 부수적으로 변경 표시돼 되돌리려 했으나 시스템이 막아 그대로 남아있음 — 실제 내용 변경은 사실상 없거나(머티리얼) Unity가 자동으로 다시 구운 폰트 아틀라스로 추정되며, 둘 다 아트 소유 영역이라 임의로 손대지 않음.
- Play 모드 확인 불가로, 메인메뉴 → 튜토리얼 씬 → 정상 시작까지 이어지는 실제 흐름은 이천서의 에디터 내 최종 확인 필요. `TutorialScene`은 아직 Build Settings 미등록(에디터 테스트엔 무관, 지시 없어 임의로 추가 안 함).

## 2026-08-07(17) 튜토리얼 시스템 — TutorialScene 완전히 제거, 메인메뉴에서 코드로 직접 시작/종료

- 이천서: 방금 만든 `TutorialScene`이 실제로는 아무 화면도 보여주지 않고 그냥 거쳐가는 자리라는 걸 짚어주자, "그럼 씬 자체가 필요 없는 것 아니냐"는 질문에 AI가 동의해 씬을 완전히 없애고 코드로 직접 처리하는 방식으로 재작업.
- `TutorialScene.unity` 삭제. 메인메뉴의 TUTORIAL 버튼이 이제 씬을 거치지 않고 코드로 즉석에서 튜토리얼 런을 만들어 바로 전투 화면으로 진입시킨다. 튜토리얼이 끝나면(마지막 대사를 클릭하면) 자동으로 메인메뉴로 돌아온다(신규 배선).
- **구현 중 저장 파일 충돌 위험을 발견해 수정**: 튜토리얼용 인스턴스가 실수로 실제 플레이어의 저장 파일과 같은 경로를 썼을 경우, 튜토리얼을 시작하는 것만으로 진짜 저장된 런이 덮어써질 뻔한 설계였다 — 튜토리얼 전용으로 디스크에 전혀 쓰지 않는 임시 저장소를 분리해 원천 차단했다.
- **구현 중 기존 버그 2건을 추가로 발견해 수정**(오늘 새로 생긴 게 아니라 이전부터 있던 것): (1) 지난번 "고쳤다"고 기록한 대사 진행 버그가 실제로는 코드 한 줄이 남아있어 재발 가능한 상태였음 — 제거함. (2) 튜토리얼 마지막 라운드가 끝나는 순간, 일반 런의 "전투 끝나면 자동으로 진행 화면 복귀" 로직이 튜토리얼에도 그대로 적용돼 있어 마지막 축하 대사가 나오기 전에 화면이 끊길 뻔했음 — 튜토리얼은 이 자동 복귀에서 제외하고 스크립트가 실제로 끝날 때만 메인메뉴로 돌아가도록 수정.
- 변경 영역: `Bootstrap/StageProgressionRuntime.cs`, `UI/MainMenu/MainMenuController.cs`, `GameScene/{TutorialDirector,GameManager}.cs`, 신규 `SaveLoad/InMemoryRunSaveFileStore.cs`.
- **검증 관련 중요 공지**: 컴파일 오류는 0으로 확인했다. 하지만 이번엔 전체 테스트 통과 개수를 보고하지 않는다 — 같은 에디터 세션에서 반복 작업(강제 리임포트, Play 모드 진입/종료) 이후 테스트 결과 자체가 오염돼(코드와 무관한 영역까지 대량 실패) 신뢰할 수 없는 상태가 됐고, AI가 이번 코드 변경을 전부 걷어내고 재검증해 이 오염이 이번 작업과 무관함(순수 HEAD 상태에서도 동일하게 재현)을 직접 확인했다. **이천서가 에디터를 재시작한 뒤 전체 테스트를 한 번 더 돌려 실제 숫자를 확인해줘야 한다.**
- Play 모드 확인 불가로, 버튼 클릭부터 종료 후 메인메뉴 복귀까지 전체 흐름은 이천서의 에디터 내 최종 확인 필요.

### 추가 수정 — 실제 플레이 중 첫 [히트] 클릭 시 크래시(StackOverflowException)

- 이천서가 실제로 플레이해보고 첫 히트 게이트에서 클릭하자마자 게임이 튕겨나가는 걸 콘솔 오류와 함께 제보. 스택 트레이스를 그대로 읽어 원인을 바로 특정 — 게이트를 닫는 코드가 "닫힘" 표시를 하기 전에 부수 효과(HUD 갱신)를 먼저 실행하는 바람에, 그 부수 효과가 다시 같은 함수를 재호출해 무한 재귀에 빠지는 순서 버그였다. 두 줄 순서만 바꿔 해결.
- 오늘 새로 만든 코드가 아니라 이전부터 있던 버그로, 실제 클릭으로 게이트가 닫히는 경로를 오늘 처음 실제 플레이에서 타면서 드러났다.
- 컴파일 0 오류 확인, 튜토리얼 관련 테스트 8개 모두 통과 확인. 전체 스위트 재검증은 여전히 에디터 재시작 후 이천서 몫으로 남아있다.

## 2026-08-07(18) 튜토리얼 시스템 — 계약 게이트 카드 사용 버그 + 애니메이션 타이밍 + 리볼버 다이얼 UX 정정

- 이천서: 실제 플레이 재확인 후 3건 제보 — 계약서만 활성화돼야 할 단계에서 다른 카드(보위 나이프)를 쓸 수 있는 버그, 여러 단계에서 애니메이션이 끝나기 전에 다음 대사가 나오면 안 된다는 원칙(전체 대본을 애니메이션 대기 지점까지 정확히 짚어 재전달), 리볼버 다이얼은 "5로 고정"이 아니라 "1부터 자유롭게 움직이되 5에 맞춰야 확인 버튼이 눌리는" 방식이어야 한다는 정정.
- **애니메이션 타이밍**: 지적받은 대기 지점들을 하나하나 코드로 확인한 결과, 대부분은 이미 구조적으로 보장돼 있었다(오늘 아침 고친 "beat 끝날 때까지 대사 안 넘어가기" 구조 덕분에 상대 반응·다음 라운드 카드딜까지 자동으로 같이 기다려짐). 유일하게 실제로 비어있던 지점은 인트로 대사가 끝나고 라운드 1 등장/카드딜 연출이 시작되기 전 — 이 둘이 "동시에" 처리되고 있어서 대사가 연출보다 먼저 뜨고 있었다. 이 지점만 정확히 짚어 고쳤다.
- **카드 사용 버그**: 오늘 아침 고친 "계약서가 안 막힘" 버그와 같은 종류 — 히트/스탠드/체인지 버튼 제한이 카드 클릭까지는 막지 못하고 있었다. 라운드 3에서 받은 카드(보위 나이프)가 계약 단계까지 손에 남아있어 계속 눌렸던 것. 계약서와 같은 방식(기본 차단, 필요한 순간만 명시적으로 해제)으로 고쳤다.
- **리볼버 UX**: 다이얼을 5로 강제 고정하던 걸, "자유롭게 움직이되 5가 아니면 확인 버튼이 안 눌리는" 방식으로 완전히 바꿨다.
- 변경 영역: `GameScene/{TutorialDirector,GameManager,GameFlowController,RevolverNumberSelectorView}.cs`, `UI/GameHudView.cs`.
- 검증: 컴파일 0 오류 확인. 세션 오염 문제로 전체 스위트 대신 관련 테스트 94개(리볼버 프레젠테이션 포함)를 targeted로 돌려 전부 통과 확인.
- Play 모드 확인 불가로, 이번 3건 모두 실제 화면에서 의도대로 동작하는지 이천서의 에디터 내 최종 확인 필요.

## 2026-08-08 튜토리얼 시스템 — 카드 오브젝트화, 라운드 전환 순서, 능력 옵션 차단, 대사 중 호버, 적 등장 연출, 겁쟁이 도박사 체력, 대사 SO화 (7건)

- 이천서: 실제 플레이 재확인 후 7건 제보 — 아스모데우스 카드가 항상 실제 오브젝트로 나와 있어야 함, 라운드 전환 대사 순서, 능력 미사용 옵션도 막아야 함, 첫 대사 자동 출력+대사 중 호버 허용, 적 등장 연출 누락, 겁쟁이 도박사 체력 오류(10 대신 4여야 함), 모든 대사를 SO로 데이터화.
- **카드 오브젝트화**: 대사가 넘어갈 때마다 카드까지 같이 사라지던 걸, 말풍선 텍스트만 숨기고 카드 오브젝트는 계속 떠 있도록 분리. 회전도 어중간하게 서 있던 걸 플레이어/적 손패 카드와 똑같이 눕는 각도로 맞췄다.
- **라운드 전환 순서**: 라운드1이 끝나면 라운드2 카드가 대사보다 먼저 화면에 나와버리던 걸, 영혼 설명 대사가 다 끝나야만 라운드2 카드가 드러나도록 순서를 뒤집었다(인트로→라운드1 전환 때 썼던 것과 같은 방식 재사용).
- **능력 옵션 차단**: 코드상으로는 이미 정상 작동하는 것으로 확인됐지만, 확실히 하기 위해 클릭 처리 지점에 재검증을 추가로 걸었다.
- **대사 중 호버**: 대사가 나오는 동안 다른 오브젝트에 마우스를 올려도 아무 반응이 없던 걸, 호버(미리보기)는 정상적으로 동작하고 클릭만 대사 진행용으로 막히도록 분리했다. 첫 대사 자동 출력과 적 등장 연출 누락은 코드를 끝까지 추적했지만 재현 가능한 원인을 찾지 못했다 — 이번 다른 수정들 반영 후에도 여전하면 다시 확인이 필요하다.
- **겁쟁이 도박사 체력**: 원래 카탈로그 자체에 체력이 3으로 잘못 들어가 있었고, 튜토리얼은 그 잘못된 값 때문에 "라운드2만으로 죽어버린다"고 판단해 별도로 10을 덮어쓰고 있었다 — 이중으로 잘못된 값이 우연히 서로를 가려온 상황이었다. 카탈로그를 4로 바로잡고 나니 10 덮어쓰기 없이도 원래 스크립트(아스모데우스 능력에서 정확히 죽음)가 그대로 맞아떨어져 그 오버라이드를 삭제했다. 관련 테스트 5건도 함께 동기화.
- **대사 SO화**: 대사 텍스트와 진행 순서를 코드에서 완전히 분리해 별도 데이터 에셋(`TutorialScript.asset`)으로 뺐다 — 이제 대사 수정은 코드를 안 건드리고 이 데이터 파일만 고치면 된다. 원본 19개 스텝을 전부 정확히 옮겨 담고 재조회로 일치 여부까지 확인했다. 다만 이 에셋을 연결하는 `GameManager` 컴포넌트가 씬이 아니라 별도 프리팹 파일(`GameManager.prefab`)에 값이 저장되는 구조라는 걸 이번에 발견해서, 그 프리팹 파일에 직접 연결했다.
- **씬 저장 관련 주의사항**: GameScene을 에디터에서 저장했더니 의도와 무관한 UI 레이아웃 값이 대량으로 바뀌는 현상을 이번에도 확인해서, 필요한 씬 변경(카드 회전)은 파일을 직접 텍스트로 수정하는 방식으로 안전하게 반영했다. 또한 이미 삭제된 튜토리얼 씬을 가리키는 끊어진 빌드 설정 참조도 발견해 정리했다.
...
## 2026-08-08(2) 튜토리얼 시스템 — "대사가 전혀 안 나옴" 근본 원인 발견 및 수정

- 이천서: "지금 튜토리얼 시작해도 아무 대사도 안 함" 제보 후, "애초에 상대 등장 연출이 안 나옴. 그래서 다음 프로세스로 안 넘어가는 듯?"라는 결정적 단서 추가.
- git 히스토리를 확인해보니 지난 7건 커밋이 실제로 `main`에 들어가 있었고, 그 사이 팀원들의 다른 커밋도 여러 건 쌓여 있었다 — 그중 하나가 GameScene을 열었다 저장하면서 나레이터 카드 회전을 예전 값으로 되돌려놨다(사소한 시각 문제, "대사 안 나옴"의 원인은 아니었음). 튜토리얼 관련 코드 자체는 그 사이 아무도 안 건드렸다는 걸 git diff로 전부 직접 대조해서 확인했다.
- 코드만 읽어서는 원인을 못 찾아서, 이번엔 처음으로 Unity 에디터를 실제로 Play 모드로 실행시켜 튜토리얼 버튼을 코드로 직접 눌러보고, 내부 상태를 실시간으로 들여다봤다. 그 결과 진짜 원인을 찾았다: 전투 화면으로 넘어가는 로직이 씬이 로드된 직후 **딱 한 번만** 시도하고, 그게 어쩌다 한 박자 늦게 준비되는 순간과 겹치면 그대로 영원히 멈춰버리는 구조였다 — 재시도하는 코드가 어디에도 없었다. 적 등장 연출과 튜토리얼 대사가 둘 다 같은 "전투 화면 진입" 로직 안에서만 시작되기 때문에, 이게 안 도니까 둘 다 동시에 안 보였던 것이다(사용자의 관찰이 정확했음).
- 매 프레임 자동으로 재시도하도록 고쳤다. 정상 케이스에는 영향 없고(이미 성공했으면 재시도 로직은 그냥 넘어감), 이번처럼 타이밍이 어긋난 경우에만 다음 프레임에 자동으로 복구된다.
- 수정 후 다시 Play 모드로 처음부터 끝까지 재현해봤다 — 버튼 클릭부터 화면 전환, 튜토리얼 시작, 실제 표시되는 대사 텍스트("드디어 정신을 차렸군.")까지 전부 직접 확인했다. 이번엔 "코드상 원인 불명"이 아니라 실제로 재현하고 고치고 다시 확인까지 전부 마쳤다.
- 변경 영역: `GameScene/GameFlowController.cs`(재시도 로직 추가), 씬(회전 재복구).
- 검증: 컴파일 0 오류, 관련 테스트 8개 전부 통과. Play 모드에서 상태 전이는 직접 확인했지만, 실제 애니메이션 타이밍/텍스트 타이핑 속도 등 시각적인 부분은 여전히 이천서의 직접 플레이 확인이 필요하다.

## 2026-08-08(2) 튜토리얼 2차 회귀 — "패배 연출 뜨고 멈춤" 원인 발견 및 수정

- 1차 수정 이후 재테스트에서 새 증상 발견: 튜토리얼 시작 시 상대 등장 대신 패배(퇴장) 연출이 먼저 뜨고, 그 뒤로 등장 연출과 대사가 멈춤. 1차 버그가 막고 있던 코드 경로가 처음으로 실행되면서 드러난 별개의 잠복 버그였다.
- 원인: 튜토리얼은 화면 전환이 다른 콘텐츠보다 한 단계 적게(단일 홉으로) 일어나서, 캐릭터가 이미 화면에 떠 있는 상태에서 "아직 준비 안 됨" 신호가 겹쳐 불필요한 퇴장 애니메이션이 발동하고, 그 여파로 진짜 등장 애니메이션이 통째로 스킵되는 구조였다. 일반 플레이는 이 두 조건이 절대 동시에 안 겹쳐서 지금까지 안 보였다.
- 수정: 이 특정 상황에서만 퇴장 애니메이션 없이 즉시 숨기도록 분기 추가. 일반 플레이 동작에는 영향 없음.
- Play 모드로 직접 재현·검증: 문제의 그 순간에 퇴장 애니메이션이 더 이상 발동하지 않고, 등장 애니메이션이 끝까지 재생되어 다음 대사로 정상적으로 넘어가는 것까지 확인했다.
- 변경 영역: `GameScene/GameFlowController.cs`. 검증: 컴파일 0 오류, 관련 테스트 11개 전부 통과. 실제 시각적 타이밍은 이천서의 직접 플레이 확인이 필요하다.

## 2026-08-08(3) 튜토리얼 3차 회귀 — "라운드1 종료 대사가 스탠드 즉시 나옴" 원인 발견 및 수정

- 2차 수정 이후 재테스트에서 새 증상 발견: "패자는 대가를 치뤄야겠지" 대사가 라운드 1이 실제로 끝나는(영혼 피해/숫자 비교 연출이 다 재생되는) 시점이 아니라, 플레이어가 스탠드를 누르는 즉시 튀어나옴.
- 원인: 게이트를 닫는 로직(다음 대사로 넘어갈지 판단하는 코드)이 "라운드 결과가 이미 확정된 최종 상태"만 보고 즉시 판단해버려서, 라운드 1의 데미지/비교 연출이 화면에서 아직 재생 중인데도 대사가 먼저 튀어나왔다. 라운드 2 카드가 미리 보이는 문제는 원래부터 별도 메커니즘으로 잘 막혀 있어서 괜찮았음 — 대사 타이밍만 문제였다.
- 수정: 그 조기 판단 호출 한 줄을 제거하고, 이미 존재하던 "연출이 전부 끝난 뒤" 판단 시점에만 의존하도록 정리. 다른 게이트(히트, 체인지, 리볼버, 계약서 등)에는 영향 없음.
- Play 모드로 직접 재현·검증: 라운드 1 연출이 재생되는 동안에는 대사로 안 넘어가고, 연출이 완전히 끝난 뒤에야 "패자는 대가를 치뤄야겠지" 이하 대사 → 라운드 2 세팅 → "한참 모자라는군..." 순서로 정확히 넘어가는 것까지 단계별로 확인했다.
- 변경 영역: `GameScene/GameManager.cs`. 검증: 컴파일 0 오류, 관련 테스트 11개 전부 통과. 실제 연출의 호흡감(대사 사이 텀 등)은 이천서의 직접 플레이 확인이 필요하다.

## 2026-08-08(4) 튜토리얼 4차 회귀 — "실제 플레이에서도 첫 등장 연출이 안 나옴" 원인 발견 및 수정

- 3차 수정 직후 이천서가 "자동화 테스트 화면이 아니라 실제 플레이에서도 등장 연출이 안 나온다"고 재차 제보. 확인 결과 튜토리얼 시작 직후 상인/적이 맨 처음 나타나는 그 순간이 문제였고, 실제로 재현되는 버그였다.
- 원인: 문이 열리는 연출이 아직 재생 중인데 플레이어가 빠르게 확인 버튼을 눌러 다음 화면으로 넘어가면, "등장 대기 중" 상태를 나타내는 플래그가 그 자리에서 무조건 초기화돼버렸다. 그 뒤 문 연출이 끝나 "이제 등장시켜도 돼"라는 신호가 와도, 이미 꺼진 플래그 때문에 그 신호를 무시해버려서 캐릭터가 영원히 안 나타났다 — 이 신호는 한 번만 오기 때문에 놓치면 복구가 안 됨.
- 이전 두 차례 자동화 검증에서 이 버그를 못 잡은 이유: 매번 문 연출을 강제로 즉시 끝내고 나서야 다음 단계를 진행했기 때문에, "문 연출이 진행 중인데 빠르게 클릭하는" 상황 자체를 한 번도 재현하지 못했다. 이번엔 강제 완료 없이 실제 타이밍 그대로 재현해서 확정했다.
- 수정: 문 연출이 아직 재생 중일 때 화면 전환으로 "숨겨야 함" 요청이 들어오면, 그 자리에서 바로 상태를 지우지 않고 "문 연출 끝나면 숨겨" 라고만 기억해두도록 변경. 문 연출은 끝까지 자연스럽게 재생되고, 끝난 시점에 정확히 의도한 대로(등장 안 시키고 숨김 상태로) 정리된다.
- Play 모드로 실제 타이밍 그대로 재현·검증 + 스크린샷으로 캐릭터가 실제 화면에 나타나는 것까지 시각적으로 확인. 이후 튜토리얼 인트로가 끝나면 정상적으로 다시 한번 등장하는 것도 확인 — 기존 동작에 영향 없음.
- 변경 영역: `GameScene/GameFlowController.cs`. 검증: 컴파일 0 오류, 관련 테스트 11개 전부 통과, 스크린샷으로 시각 확인까지 완료.

## 2026-08-08 자동 발동 결과 안내 유지·거짓말 탐지기 정보 표시

- 이천서: 실제 플레이에서 자동 발동 결과가 읽기 전에 사라지는 문제를 제보하고, 라운드 진행 중에는 다음 플레이어 행동 또는 새 자동 결과까지 유지하되 카드 비교 연출 시작 시 제거하는 기준을 확정했다. 선택 안내는 임시로 덮은 뒤 기존 결과가 다시 보이도록 결정했으며 거짓말 탐지기의 `이상/미만` 정보 위치를 검토·승인한다.
- AI 보조: 결과 생성부터 턴·라운드 종료까지의 삭제 경로를 추적하고, 일반 턴은 플레이어 공개 행동 경계로 수명을 이동하되 라운드 확정 시 비교 연출 게시 전에 제거하는 코드·테스트·문서화를 보조했다. 플레이어 탐지기만 비교 결과를 받고 실제 숨은 숫자와 적 전용 정보는 노출하지 않는 기존 경계를 재검증했다.
- 변경 영역: `CoreLoopBattle.cs`, 자동 카드 통합·안내 테스트, 자동 카드 명세·진행 및 AI 활용 기록. 집중 EditMode 13/13, 전체 1,213/1,231 통과, 컴파일 오류 0. 잔여 18건은 기존 범위 밖 실패이며 다른 팀원 기여는 확인되지 않아 배정하지 않았다.
## 2026-08-08 전투 종료 HUD·턴 표시·튜토리얼 도입 순서 역할

- 이천서: 스테이지 종료 HUD 비노출, 턴 소유자 영문 표기, 튜토리얼의 첫 대사 우선 순서, 아스모데우스 카드·말풍선 배치, 능력 미사용 선택 차단 기준을 결정하고 최종 플레이 승인한다.
- AI: HUD 프레젠테이션, 튜토리얼 전투 바인딩·등장 대기, 계약 옵션 제한, 내레이터 최초 활성화 수명 문제를 분석하고 코드·씬·프리팹·회귀 테스트를 좁게 수정했다. Unity MCP로 컴파일, 관련 149건, 전체 1,241건 및 1280×720 화면을 검증했다. 관련 기능 테스트는 통과했으며 전체 스위트의 기존 에셋·프로필 기대값 실패는 범위 밖으로 기록했다.
## 2026-08-08 나이프·튜토리얼 재보완 역할 기록

- 이천서: 재현 증상, 원하는 연출 순서, 자동 카드 중 나이프 유지 정책과 튜토리얼 입력 요구 정의.
- Codex: 나이프 타임라인 상태 누수와 튜토리얼 참조/게이트 원인 분석, 구현, 회귀 테스트 및 Unity MCP 검증 담당.
- 검증: 관련 fixture 85/85 통과. 전체 EditMode 1,230/1,248 통과. 기존 범위 밖 실패 18건, 컴파일 오류 0건.

## 2026-08-08 나이프 재사용·독극물·튜토리얼 시작 역할 기록

- 이천서: 두 번째 나이프 재현 조건, 나이프 버스트 비교 생략, 첫 라운드 독극물 연출, 새 튜토리얼 순서와 계약 카드 보존 요구 정의.
- Codex: action ordinal 오결합 원인 분석, 버스트 원인 분기, 독극물 페이드/첫 라운드 재생, 입장 우선 튜토리얼 흐름과 계약 카드 보존 구현, 회귀 검증 담당.
- 검증: 관련 EditMode fixture 73/73 통과. 전체 EditMode 1,234/1,252 통과, 기존 범위 밖 실패 18건. 컴파일 및 Console 코드 오류 0건. 실제 Play Mode 반복 검증은 미수행.
## 2026-08-08 리볼버·선택 UI·첫 라운드 보완 역할 기록

- 이천서: 연속 리볼버, 화염방사기 버튼/카드 중복 클릭, 튜토리얼 첫 Hit, 집행자 첫 라운드 독극물의 실제 재현 조건과 기대 순서 정의.
- Codex: 원인 분석, C#/GameScene 직렬화 수정, 회귀 테스트 작성, Unity MCP 컴파일 및 EditMode 검증.
- 검증: 신규 테스트 2/2, 관련 fixture 108/109, 전체 EditMode 1,236/1,254 통과. 관련·전체 실패는 기존 기준 불일치 18건으로 신규 실패 없음.

## 2026-08-09 상대 선택·바알제붑·엘리트 정합성 역할 기록

- 이천서: 기능 요구, 사기꾼 엘리트 판정, 바알제붑 연출 순서와 최종 승인 책임.
- Codex: 후보 생성·복원 경로 조사, Quick Play 디버그 진입 구현, 적 profile/asset/ViewModel 정합성 대조, 버스트 대체 단계 분리, 테스트와 Unity MCP 검증 보조.
- 변경 범위: 상대 선택 생성기, 적 프로필 기본 카탈로그, CoreLoop 악마 계약 처리, GameScene 디버그 진입, Editor Quick Play, 관련 테스트·문서.
- 검증: 관련 테스트 통과, StageProgression 287/289, 전체 EditMode 1,235/1,254, 컴파일 오류 0건. 잔여 19건은 기존 범위 밖 자산·상점 기준 불일치.
- 외부 에셋·오픈소스·패키지 추가 없음. 최종 승인 책임은 이천서에게 있다.

## 2026-08-09 바알제붑 치명 효과 실행 장벽 역할 기록

- 이천서: 처치 성공 시 재현되는 선행 발동 문제 제보, 사탄 포함 효과 연출 완료 후 바알제붑 실행 기준 정의, 최종 승인.
- Codex: 스냅샷 분리만으로 규칙 실행이 멈추지 않는 원인 분석, CoreLoop continuation 장벽과 GameScene 재개 연결, 적 턴 중복 방지, 리볼버·나이프·사탄 회귀 작성 및 Unity MCP 검증.
- 검증: 관련 EditMode 29/29 통과. 전체 EditMode 1,237/1,256 통과, 기존 범위 밖 실패 19건, 컴파일 오류 0건. 외부 에셋·오픈소스·패키지 추가 없음.
- 후속 역할: 이천서는 적 비공개 리볼버 적중의 실제 화면 순서를 재제보했다. Codex는 pending 효과가 결과 cue를 가리는 프레젠테이션 조건을 재현·수정하고, 장벽 앞 세 공격 연출 cue와 턴 정지를 검증했다. 관련 87/87, 전체 1,237/1,256이며 신규 실패와 컴파일 오류는 없다.

## 2026-08-09 나이프·적 바알제붑·턴 표기 역할 기록

- 이천서: 재발 조건과 `리볼버 선언·명중 → 바알제붑 발동`, 한글 턴 문구 기준 정의 및 최종 승인.
- Codex: 나이프 Animator 상태 누수와 action ordinal pairing 분석, 적 소유 바알제붑 재현 테스트 작성, 결과 cue 우선순위와 완료 장벽 보완, HUD 표기 변경, Unity MCP 검증.
- 검증: 관련 fixture 200/200, 전체 EditMode 1,250/1,267 통과. 기존 비관련 실패 17건, 게임 코드 컴파일 오류 0건. 외부 에셋·오픈소스·패키지 추가 없음.

## 2026-08-09 턴 주기·나이프 중복 회귀 역할 기록

- 이천서: `1턴` 시작, 양쪽 행동 한 쌍의 턴 정의, 나이프 3회 재생 실플레이 조건 제보.
- Codex: CoreLoop 턴 증가 경계 수정, 스탠드 후 연속 행동 처리, 나이프 결과 소비·복구 idempotency 보완, 회귀 및 전체 Unity 검증.
- 검증: 관련 fixture 221/221, 전체 EditMode 1,251/1,268 통과. 기존 비관련 실패 17건, 게임 코드 컴파일 오류 0건.

## 2026-08-09 상점 경제 밸런스 역할 기록

- 이천서: 적별 골드, 일반·악마 카드 가격, 특수 행동 가격·회복량, 최종 보스 0골드 확정.
- Codex: SO·런타임 fallback·특수 행동 기본값 정합화, 0골드 검증 경계 수정, 관련 테스트와 문서 갱신, Unity MCP 전체 회귀 검증.
- 검증: 관련 48/48 및 추가 8/8 통과. 전체 EditMode 1,253/1,270 통과, 기존 비관련 실패 17건, 게임 코드 컴파일 오류 0건.

## 2026-08-09 튜토리얼 대본·리치 텍스트·하이라이트 역할 기록

- 이천서: 전체 튜토리얼 대본, 강조 표기 문법, 행동 하이라이트 대상, 아스모데우스 카드 호버 문구, 라운드 시작·정리 순서를 정의했다. 후속으로 W/D 카메라 안내와 두 번째 페이즈 Hit 문구를 확정했다.
- Codex: 마크업 파서, 원형 스포트라이트, 대본 SO, 계약 전 카드 수명·호버, 화자 전환, 2초 지연과 라운드 표시 보존을 구현하고 회귀 테스트를 추가했다.
- 검증: 튜토리얼 관련 EditMode 18/18 통과. 전체 1,282개 중 기존 비관련 실패 21건, 수정 스크립트 진단 오류와 C# 컴파일 오류 0건. Play Mode 시각 검증은 미수행이다.

## 2026-08-09 튜토리얼 진행 타이밍·호버·행동 해골 역할 기록

- 이천서: 색상 종료 위치, 상대 턴·연출 대기 기준, 내 덱 확인 단계, 리볼버 숫자 선택 순서, 호버 유지, 양쪽 행동 해골 표시, 아스모데우스 최종 대사 순서를 확정했다.
- Codex: 마크업 색상 경계, 덱·리볼버 게이트, 전투 프레젠테이션 장벽, 대사 중 호버 전용 경로, 행동 해골 표시 보존과 회귀 테스트를 구현했다.
- 검증: 관련 4/4, 12/12, 17/17, 25/25 통과. 전체 EditMode 1,268/1,287, 기존 비관련 실패 19건, Console C# 컴파일 오류 0건이다. Play Mode 시각 검증은 전환 정체와 기준 이미지 부재로 미수행이다.

## 2026-08-09 튜토리얼 리볼버·상대 연출·해골 역할 기록

- 이천서: 리볼버 설명 진입 시점, 일반 전투와 동일한 행동 해골 수명, 상대 행동 조기 표시 제거를 확정했다.
- Codex: 리볼버 UI/Pending 효과 게이트, 전투 프레젠테이션 평가 장벽, 재렌더 예약, 라운드 전환 해골 초기화 보류를 구현하고 회귀 테스트를 추가했다.
- 검증: 신규 10/10, 영향 회귀 46/46, 전체 EditMode 1,281/1,300 통과. 기존 비관련 실패 19건, C# 컴파일 오류 0건이다.

## 2026-08-09 튜토리얼 하이라이트·말풍선 후속 역할 기록

- 이천서: 플레이어 행동 종료, 리볼버 선택, 계약 후보 UI 진입별 하이라이트 제거 시점과 드로우 덱 강조 강도, 계약 전후 말풍선 이동·반전 방향을 확정했다.
- Codex: 대상 일치형 하이라이트 제거, 드로우 덱 전용 스포트라이트 스타일, 화자별 오프셋, 말풍선 시각 루트와 텍스트 상쇄 반전, 실화면 기준 내부 좌표 `(-115, 0)`/`(126.6, 30.743)`, 회귀 테스트와 GameScene 직렬화를 구현했다.
- 검증: 신규 5/5, 전체 EditMode 1,299/1,318 통과. 잔여 19건은 기존 비관련 기준선이며 C# 컴파일 오류는 0건이다.

## 2026-08-10 MMUI-02 메인 메뉴 시작 안내 연출

- HONG: 앱 실행 뒤 첫 메인 메뉴에서만 그래픽 가속·전체 화면 권장 문구를 순차 표시하고, 검정 화면 공개 시점에 BGM을 시작하도록 연출과 문구를 확정했다.
- Codex: 검정 화면을 한 프레임 먼저 렌더하고 0.25초 안정 구간을 둔 `MainMenuView`의 6.95초 unscaled DOTween 시퀀스(문구당 3초, 줄 간격 50), 전체 화면 검정 오버레이와 Telegraph 입력 잠금을 구현했다. 페이드 생성 중 뒤쪽 트윈이 알파를 미리 덮어쓰던 문제를 명시적 시작·종료 값 트윈으로 수정했다. 안내가 완전히 끝난 뒤 `MainMenuController`가 무드·BGM·Telegraph 입장 연출을 함께 시작하며, 같은 Play 세션의 메인 메뉴 재진입은 안내를 생략한다.
- 최종 검증: `MMUI02` 2/2 통과(job `25d88c7407864a27a421c6d06d6858a4`), `MMUI01` 5/5 통과(job `fcfeab3963ed435498717aa6271cc203`), `MainMenuScene` validation issue·missing script·broken prefab 0건, C# 컴파일 오류 0건이다. Play 모드 첫 페이드의 알파를 시작·중간·완료 지점에서 실측해 `0.000 → 0.500 → 1.000`을 확인했다.
