...
| --- | --- | --- | --- |
| 이천서 | 프로젝트·기획 문서, 전투 코어·적 프로필·상대 선택과 전투 정보 UI 통합 담당 | 규칙·구조 결정, 적 5종 프로필·AI·카드 효과·전투 변환, 상대 선택·등급별 적 정보 UI, 테스트·완료 기록 관리 | 코어 루프·런 진행·전투 행동·카드 사용·전투 보상 완료, EP-06 적 프로필과 EUI-05 상대 선택·전투 정보 UI 1차 범위 완료(AI 보조) |
...
| 2026-07-22 | 악마 계약 DC-01 | 네 악마 정의·카탈로그, 런 최초/현재 악마 덱, 후보 3장과 버림 보충 전투 덱, 런→전투 변환·독립 시드 구현 관리 | `CoreLoop/DemonContracts`, `RunDemonDefinition.cs`, `PlayerRunState.cs`, `StageBattleFactory.cs`, `CoreLoopBattle.cs`, DC-01 테스트·기록 문서 | AI 구현·테스트·Unity MCP·기록 보조, 대상 19/19·전체 EditMode 329/329·Console 0; CU-M03 표시 회귀도 코드 투영만 복구, 씬·프리팹·패키지·외부 에셋 변경 없음, 이천서 최종 승인 대기 |

코어 루프 1~4단계, 독립 런·스테이지 SP-00~SP-04, 전투 행동 BA-00~BA-05, 카드 사용 CU-00~CU-06과 전투 보상 RW-00~RW-05의 구현·검증·문서 마감이 완료되었다. 이천서는 EP-00부터 적 5종 전투 프로필·AI·카드 효과·실제 전투 변환을 담당하며 EP-01에서 프로필·미리보기·정책 기반과 결정적 전투 설정, EP-02에서 공개 관측·유효 행동·재검증과 적 카드 실행 경계, EP-03에서 공개 숫자 추론과 일반 적 3종 정책, EP-04에서 집행관 해머·나이프 후속 평가와 엘리트 표시·보상 연결, EP-05에서 최종 보스 3구간 정책·강행동 예고·런 승리, EP-06에서 선택 키의 실제 전투·보상 변환과 반복 상태 격리를 구현해 적 전투 프로필 1차 범위를 마감했다. EUI-00부터 상대 후보 생성·선택 UI와 전투 중 등급별 적 정보 UI도 이천서 담당으로 변경했으며, EUI-01 후보 기반, EUI-02 선택 화면, EUI-03 실제 전투·보상 통합, EUI-04 등급별 전투 정보, EUI-05 반복 전체 흐름·씬·화면 검증까지 완료해 상대 선택·전투 정보 UI 1차 범위를 마감했다. RF-00에서 이천서는 최신 골드·상점 규칙과 HONG 인계용 문서 4종을 작성했다. 아트는 Shim0Hwan, 골드·상점과 정식 런 진행 구현은 HONG의 계획 영역으로 유지한다.
...
| DC-02 | 영혼 비용·횟수·후보 3장·선택 상태·세션 전달 | 이천서(AI 구현·검증 보조 예정) | 미착수 |
| DC-03 | 벨페고르 수직 기능과 정보 은닉 | 이천서(AI 구현·검증 보조 예정) | 미착수 |
| DC-04 | 마몬 주사위·레비아탄 리볼버 연동 | 이천서(AI 구현·검증 보조 예정) | 미착수 |
| DC-05 | 플레이어 계약 화면·런 영혼·보상 연결 | 이천서(AI 구현·화면·검증 보조 예정) | 미착수 |
| DC-06 | 사탄 규칙 확정·카운터·권능 카드 구현 | 이천서 | 기획 확인 대기 |
| DC-07 | 적 AI 계약·반복 회귀·실제 흐름·기록 마감 | 이천서(AI 구현·검증·기록 보조 예정) | 미착수 |
