...
![2.5D 테이블 배치 콘셉트](./images/table-layout-concept-v2.png)
...
- 위협용 해머는 카드나 테이블을 강타해 공개 카드를 버리고 상대의 스탠드 상태를 무너뜨리는 연출로 표현한다.
...
적을 퇴장시키고 같은 자리에 상인을 등장시킨다. 상인은 판매 카드, 휴식과 카드 제거 등 현재 이용 가능한 상품·서비스를 테이블에 펼친다. 상품 가격과 보유 골드는 화면 UI와 테이블 가격표로 함께 표시할 수 있다.
