...
> 작업 식별자: RF-00~RF-05  
> 버전: v1.3
> 상태: 시작 악마 2장 자동 지급 재명세 · RF 기존 선택 흐름 이관 대기
> 최종 갱신: 2026-07-31
...
> **시작 악마 지급 변경 (2026-07-31):** 새 런은 기본 풀에서 서로 다른 2장을 무작위로 생성해 모두 악마 덱에 지급한 뒤 첫 상대 선택으로 이동한다. 시작 선택 UI와 입력은 제거하며 지급 결과 저장이 성공하기 전에는 첫 상대 선택을 열지 않는다. 기존 2장 제안·1장 선택 구현은 과거 검증 이력이다.
...
- 정식 Runtime은 새 런의 결정적 루트 시드에서 서로 다른 악마 후보 2장을 만들고, 1장 선택과 체크포인트 저장이 끝난 뒤 첫 상대 제안을 준비한다.
