...
> 작업 식별자: RF-00~RF-05  
> 버전: v1.1
> 상태: RF-01~RF-05 코드·자동 회귀 완료 · Unity 실화면 검증 대기
> 최종 갱신: 2026-07-30
...
- `StageProgressionRuntime`의 프로토타입 플레이어는 빈 악마 덱으로 시작한다.
- 새 런의 결정적 루트 시드에서 서로 다른 악마 후보 2장을 만들고, 1장 선택과 체크포인트 저장이 끝난 뒤 첫 상대 제안을 준비한다.
- 일반 전투 전 상대 후보는 정확히 2명이며, 선택한 프로필 키만 실제 `CoreLoopBattle` 생성에 사용한다.
- 활성 정식 전투 없이 `GameScene`을 직접 실행하면 `StageTest`로 이동한다.
- 고정 상대 독립 전투는 `allowStandaloneBattleForDebug`가 켜진 경우에만 허용한다.
- 저장·이어하기는 기존 루트 시드와 시작 악마 선택을 그대로 복원한다.
...
| 날짜 | 작성자 | 변경 내용 |
| --- | --- | --- |
| 2026-07-30 | 이천서 | RFM02 실제 진입 계약을 추가했다. Runtime 생성기에 시작 악마 후보 생성기를 주입하고 빈 악마 덱에서 시작하며, GameScene 직접 실행은 정식 런으로 이동하고 독립 총잡이 전투는 디버그 옵션으로 제한한다. |
