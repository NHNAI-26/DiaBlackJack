...
> 작업 식별자: RF-00~RF-05  
> 버전: v0.4
> 상태: RF-01A·RF-01B 순차 착수 가능 · 여섯 프로필 지급량 확정
> 최종 갱신: 2026-07-29
...
> 시작 악마 선택, 전투 보상·골드 정산, 상점 나가기, 사건 해결과 런 종료 뒤에만 체크포인트를 갱신한다. 실제 저장 파일·버전 마이그레이션은 기존 RF-00~RF-05 범위에 구현되지 않았으며 `Docs/formal-run-flow-design.md` v0.7과 저장 시스템 v0.9를 기준으로 별도 이관한다.
