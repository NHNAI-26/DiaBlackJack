...
> 작업 식별자: RF-00~RF-05  
> 버전: v1.3
> 상태: 시작 악마 2장 자동 지급·저장·공개 후 자동 전환 구현 완료
> 최종 갱신: 2026-07-31
...
> **시작 악마 지급 구현 (2026-07-31):** `StartingDemonGrantGenerator`가 기본 풀에서 서로 다른 2장을 결정적으로 생성한다. `StageProgressionSession.TryStartRun()`은 미지급 상태에서 두 장을 원자 지급하고 같은 세션에서 재호출해도 다시 추첨하지 않는다. `TryCompleteStartingDemonReveal()`이 공개 연출 완료를 받아 체크포인트 저장을 요청하며, 저장 성공 뒤 상대 선택으로 자동 전환한다.
