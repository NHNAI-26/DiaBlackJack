...
> 현행 기획은 일반 카드 3장·악마 카드 2장, 슬롯별 재고 1개와 구매 후 `SOLD OUT`, 방문 전체 카드 구매 상한 없음, 새로고침·재입고 없음이다. 라이터와 위스키는 방문당 각각 1회이며 둘 다 이용할 수 있다. 라이터는 사용한 방문마다 다음 상점 가격이 20골드 오르고, 위스키는 50골드로 고정된다.
...
RF-02 밸런스 기본값은 카드별 `basePurchasePrice`, 라이터 50골드·사용 방문당 +20골드, 위스키 50골드·영혼 2 회복이다. 적 처치 골드는 겁쟁이 도박사 100, 총잡이 120, 광신도 200, 사기꾼·집행자 300, 최종 보스 0이다. 엘리트 승리 가중치는 숫자 5 이상 일반 카드의 선택 가중치를 2배로 적용하며 슬롯 수와 가격은 유지한다.
...
- 최종 보스 공개에서 준비한 `bossStage` BGM은 다른 적과 같은 전투 캐릭터 입장 시작에 한 번 재생하고 pending을 즉시 소모한다.
- 튜토리얼 첫 리볼버 적중은 양측 비공개 카드 플립과 결과 홀드 완료 뒤 라운드 종료 안내 대사를 시작한다. 계약서 승인 입력은 같은 프레임에 계약서 하이라이트를 제거하고 계약서와 아스모데우스 화자 카드를 0.35초 페이드한다. 거부 입력은 표시를 유지한다.
