...
> 기획·개발 책임자: 이천서
> 버전: v0.2
> 상태: RW-00~RW-05 구현·검증 완료 · 적 6종 등급 매핑은 EP-R04 연계
> 최종 갱신: 2026-07-29
...
Notion v0.7의 네 일반 프로필은 `LowGrade`, 집행자는 `HighGrade`, 최종 보스는 `HighGrade` 기대 등급을 유지한다. 새 프로필 값은 EP-R04에서 `ExpectedRewardTier`와 함께 검증하며, 골드 수치는 본 카탈로그에 추가하지 않는다.
