...
> 기획·개발 책임자: 이천서
> 버전: v0.3
> 상태: RW-00~RW-05 구현·검증 완료 · 적 6종 등급·골드 명세 확정
> 최종 갱신: 2026-07-29
...
Notion v0.7의 네 일반 프로필은 `LowGrade`, 집행자는 `HighGrade`, 최종 보스는 `HighGrade` 기대 등급을 유지한다. 새 프로필 값은 EP-R04에서 `ExpectedRewardTier`와 함께 검증한다. 승리 골드는 겁쟁이 도박사 3·총잡이 4·광신도 6·사기꾼 7·집행자 9·최종 보스 15이며, 본 카드 보상 카탈로그가 아니라 RF-01B의 `GoldRewardCatalog`에서 관리한다.
