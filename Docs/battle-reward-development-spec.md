...
> 기획·개발 책임자: 이천서
> 버전: v0.4
> 상태: RW-00~RW-05 구현·검증 기록 보존 · 정식 런 사용 중단 명세, 코드 이관 대기
> 최종 갱신: 2026-07-30

## 1. 기술 목표

이 문서는 일반·엘리트·최종 보스 전투 승리 뒤 카드 후보 3장을 생성했던 순수 C# RW 구현의 기술 계약을 보존한다. 현행 정식 런은 이 계약을 진행 필수 경로로 사용하지 않는다.

기존 RW 경로에서는 일반·엘리트 전투가 보상 완료 뒤 `StageCleared`, 최종 보스가 보상 완료 뒤 `RunVictory`가 된다. 개정 정식 런은 별도 무카드보상 완료 경계를 사용해 일반전 승리를 `StageCleared`와 상점으로, 보스 승리를 `RunVictory`로 직접 보낸다. 기존 RW 타입과 테스트는 삭제하거나 의미를 바꾸지 않는다.

기존 프로필의 `ExpectedRewardTier`는 RW 회귀와 향후 사건 재사용을 위해 유지한다. 정식 런의 엘리트 차별화는 더 많은 골드와 높은 등급 상점 상품 가중치로 옮기며, 승리 골드는 RF-01B의 `GoldRewardCatalog`에서 관리한다.
...
| 날짜 | 작성자 | 변경 내용 |
| --- | --- | --- |
| 2026-07-30 | 이천서 | RW 기술 계약을 과거 구현·향후 사건 재사용 명세로 전환했다. 정식 런은 무카드보상 완료 경계로 `RewardSelection`을 우회하고, 일반전은 상점·보스는 즉시 승리로 이동하도록 명시했으며 기존 타입·API·테스트 제거는 별도 승인 전 금지했다. |
...
| 2026-07-29 | 이천서 | 적 6종 승리 골드 3·4·6·7·9·15를 RF-01B `GoldRewardCatalog`의 책임으로 확정하고 RW 카드 보상 카탈로그와 분리 |
