...
## 2026-08-09 악마 도감 문구·상대 자동 카드 표시 분리

- 악마 카드 SO에 도감 전용 효과·대가 override를 추가했다. 빈 값은 기존 계약 카드 문구로 대체하며 도감 런타임·Inspector 프리뷰만 새 값을 읽는다.
- 상대 시작 덱 도감에서 자동 발동 카드가 클로버 묶음으로 갈라지던 계산을 실제 전투 덱과 같이 스페이드 고정으로 맞췄다.
- 신규 회귀 6/6, 도감·대사 영향 범위 76/78이 통과했다. 실패 2건은 기존 말풍선 앵커·카메라 mask 기대값 불일치다.
- 전체 EditMode는 1,325/1,345였다. 기존 실패 19건 외 조명 블렌드 1건이 일시 실패했으며 단독 재실행 1/1 통과했다. C# 컴파일 오류는 0건이고 Console에는 기존 MCP WebSocket 예외 1건이 남아 있다.
