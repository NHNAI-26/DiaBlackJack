...
> **Notion v0.7 영향 기록**
> 적 6종의 전투 중 덱·능력·계약·주입 카드는 저장하지 않는 기존 원칙과 일치한다. 여섯 프로필 골드 값은 미정이며 RF 실제 상태도 없으므로 SV-06은 계속 미착수다. 기존 SV-01~SV-05 완료 증거와 스키마는 변경하지 않았다.
...
| 골드·상점·사건 | 흐름·서비스 가격 단계 확정, 적 6종 지급량과 실제 전체 상태 API 미완료 |
