...
> 작업 식별자: SV-00~SV-06
> 버전: v1.3
> 현재 단계: 시작 악마 2장 자동 지급 저장·복원 구현 완료
> 다음 단계: 기존 선택 체크포인트·예약·UI를 자동 지급으로 이관
> 최종 갱신: 2026-07-31

> **2026-07-30 기획 변경 기록**
> 정식 런의 카드 보상은 제거됐지만 현행 SV-04는 아직 카드 보상 성공 콜백을 저장 트리거로 사용한다. 새 승리 골드 정산·보스 즉시 승리 트리거는 RF 구현 전이므로 SV-06 미완료 상태를 유지하고 기존 완료 증거를 변경하지 않는다.

> **2026-07-31 기획 변경 기록**
> 시작 악마는 기본 풀에서 서로 다른 2장을 한 번만 추첨해 모두 지급한다. 선택 입력을 제거하고 두 장 동시 공개 완료 뒤 `StartingDemonGranted` 체크포인트를 저장한다. 스키마 v2는 두 악마와 지급 완료 여부를 보존하며 예약 재접속·이어하기에서 재추첨하지 않는다. 전체 EditMode 784/784로 검증했다.

> **RF-01A 골드 저장 활성화 (2026-07-30)**
> `PlayerRunState`가 골드를 소유하면서 캡처의 고정 0과 복원의 양수 차단을 제거했다. 현재 골드는 저장·복원되고 새 런 재시작 시 0으로 돌아간다. RF-01B 승리 정산, RF-02 상점과 RF-03 정식 순서 도메인도 완료됐으며, RF-04/05 뒤 SV-06 체크포인트 연결은 아직 남아 있다.

> **Notion v0.7 영향 기록**
> 적 6종의 전투 중 덱·능력·계약·주입 카드는 저장하지 않는다. 여섯 프로필 골드는 3·4·6·7·9·15로 확정했지만 RF 실제 상태가 없으므로 SV-06은 계속 미착수다. 기존 SV-01~SV-05 완료 증거와 스키마는 변경하지 않았다.
...
| `PlayerRunState` | 영혼·골드·일반/악마 덱 보유, 마지막 발급 ID와 복원 경계 구현 |
...
| 골드·상점·사건 | 골드 상태·저장 왕복 구현, 승리 정산·정식 상점·사건 미구현 |
...
| SV-06 | 이천서(AI 통합·반복·기록 보조) | 상점 통합 완료·잔여 검증 | 신규 5건·저장/정식 런 59/59·전체 798/798·게임 코드 오류 0 |
...
- SV-03 당시 `PlayerRunState`가 소유하지 않던 양수 골드는 `InvalidGold`로 거부했다. RF-01A에서 이 제한을 해제해 검증된 양수 골드를 복원한다.
...
- 현행 Runtime 새 런은 사탄·벨페고르·바알제붑·마몬 4장 악마 덱으로 즉시 활성화되며 시작 선택 예약을 만들지 않는다. 빈 악마 덱과 선택 생성기를 명시한 호환 경로에서는 기존처럼 결정적 후보 2장을 표시하고 선택 체크포인트 성공 뒤 예약을 제거한다.
...
- `FormalRunSession`의 일반전 정산 완료, 상점 나가기, 보스 승리·패배를 안정 체크포인트 신호로 분리했다.
- Runtime이 해당 신호를 `RunSaveFlow`·`RunSaveCoordinator`에 연결한다. 전투 규칙과 상점 도메인은 파일 저장을 직접 알지 않는다.
- 전투 정산 체크포인트에는 실제 골드·영혼·일반/악마 덱·마지막 ID와 현재 완료 상점 수·공통 서비스 가격 단계를 기록한다.
- 상점 나가기는 다음 스테이지 진입 전에 저장하고, 복원 시 Factory가 다음 상대 선택 또는 보스를 한 번만 준비한다.
- 이어하기는 저장된 루트 시드로 이전 상점 제안을 재생해 다음 `OfferId`·상품 순서를 보존하고, 저장된 가격 단계를 다음 상점의 라이터·위스키 양쪽에 적용한다.
- 저장 실패 보류 중 상점 거래·나가기를 차단했다. 기본 악마 덱 런의 빈 시작 악마 키는 역직렬화 시 `null`로 정규화했다.

### 10.2 검증

- SV06 신규 5/5: 정산/나가기 저장 필드, 나가기 복원 1회 전진, 가격 단계·상품 복원, 안정 신호 순서, 동일 체크포인트 10회 새 흐름 복원.
- 저장·정식 런 대상 59/59: Unity MCP job `4982acb2c3bf4474829ed0e5260b2278`.
- 전체 EditMode 798/798: Unity MCP job `b1f7b316f36a409b938e63c19d56102c`.
- Console 오류 분류 1건은 Test Framework 결과 XML 저장 안내이며 컴파일·게임 코드 오류는 0이다.
- 신규 흐름은 `RunSaveFlow` 객체를 10회 새로 만들어 같은 루트 시드·상점 수·가격 단계·스테이지·카드 ID를 복원했다.
- Unity 컴파일 오류와 Console Error 0을 확인했다.

### 10.3 잔여 범위

- 사건 시스템이 없으므로 `EventResolved` 실제 체크포인트는 구현하지 않았다.
- 실제 운영체제 앱 프로세스 종료·재실행과 1280×720·1920×1080 수동 화면 확인은 미검증이다. 객체·직렬화 경계 재구성 10회와 Unity Editor 회귀만 완료했다.
- 씬·프리팹·Packages·외부 에셋·오픈소스는 변경하지 않았다.

### 10.4 권장 커밋 제목

`feat : 런을 다시 열어도 상점 진행과 가격이 달라지지 않게 하다`
...
| 날짜 | 작성자 | 변경 |
| --- | --- | --- |
| 2026-07-31 | 이천서 | SV-06 정식 런 정산·상점 나가기·터미널 체크포인트와 완료 상점 수·공통 가격 단계 복원을 구현했다. 저장/정식 런 59/59·전체 798/798·Console 0을 확인했고 사건·실제 앱 재실행·두 해상도는 잔여로 분리했다. |
| 2026-07-30 | 이천서 | 새 런마다 루트 시드를 새로 공급하고 예약·정식 저장에 같은 시드를 보존하도록 `RunSaveFlow`와 SV05 회귀를 보정했다. Border·CoreLoop 테스트·StageProgression 테스트 어셈블리 직접 컴파일 성공; 저장 회귀는 Unity `JsonUtility`가 필요한 탓에 외부 Mono 실행이 불가했고 MCP 미노출 상태라 Unity Test Runner 전체 실행은 미검증으로 유지 |
| 2026-07-30 | HONG | 기본 새 런의 시작 악마 예약을 생략하고 4장 기본 악마 덱 세션을 즉시 활성화하도록 `RunSaveFlow`를 조정했으며, 호환 시작 선택 경로와 예약 제거 동작을 전용 회귀에 추가 |
| 2026-07-30 | 이천서 | RF-01A에서 골드 상태·지급·소비·재시작과 스냅샷 캡처·양수 복원을 구현하고 대상 7/7·두 어셈블리 컴파일을 확인; 전체 Unity 회귀와 SV-06 정산 트리거는 미실행·미완료로 분리 |
| 2026-07-30 | 이천서 | 카드 성장의 상점 통합으로 SV-06 체크포인트 입력이 승리 골드 정산과 보스 즉시 승리로 바뀌었음을 기록했다. 현행 SV-04 카드 보상 트리거와 검증 결과는 실제 구현 사실로 보존했고 코드·테스트는 변경하지 않았다. |
| 2026-07-29 | 이천서 | 이후 기획 결정으로 적 6종 골드가 3·4·6·7·9·15로 확정됐음을 추가 기록; RF 구현 전이므로 SV-06은 계속 미착수이며 기존 완료 증거는 변경하지 않음 |
| 2026-07-29 | 이천서 | Notion v0.7 적 전투 임시 상태 비저장·RF 골드 미정 영향을 기록하고 기존 SV-01~SV-05 증거와 스키마를 유지; SV-06 미착수 |
| 2026-07-29 | 이천서 | RFM01에서 확정한 이용 방문당 라이터·위스키 공통 가격 단계를 SV-06 상점 나가기 저장·복원 요구에 추가; 실제 RF 상태 API가 없어 구현·완료로 기록하지 않음 |
