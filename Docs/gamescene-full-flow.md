...
- 기존 `OpponentSelectionOverlay.prefab`은 변경하지 않고 GameScene의 `UIOpponentSelection_Legacy` 비활성 인스턴스로 보존한다. 활성 상대 선택 표시는 별도 `OpponentSelectionWorld.prefab`을 사용한다.
- 월드 포스터 Canvas는 테이블 표면 `(0, 3.57, 16)`에 평평하게 놓이며 기존 현상수배 포스터 프리팹 2장을 재사용한다. 두 포스터는 상대편 테이블 바깥에서 플레이어 방향으로 `0.9초 / OutCubic` 동시 이동한다. 포스터 로컬 스케일은 `1.7`, 최종 Y는 `0`으로 두어 16:9의 720p와 1080p에서 화면 높이 약 79%를 차지한다.
...
- `GameFlowController`의 직접 상대 선택 API는 월드 View의 준비 상태를 계속 확인하므로 연출 중 우회 확정할 수 없다. 퇴장 완료 콜백만 현재 Offer·후보 키·화면·저장 입력 잠금을 다시 검증하는 내부 경로를 사용한다. 확정이 거절되면 같은 Offer의 진입 연출을 복구하고 다시 선택 가능하게 한다. 최종 보스 현상수배 표시는 기존 경로를 유지한다.
