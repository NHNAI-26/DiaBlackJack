...
## GSV20 상대 선택 월드 포스터 진입 연출

- 기존 `OpponentSelectionOverlay.prefab` 에셋은 변경하지 않고 회귀 참고용으로 보존하되 GameScene의 `UIOpponentSelection_Legacy` 인스턴스는 제거한다. 일반 상대 선택과 최종 보스 공개 모두 `OpponentSelectionWorld.prefab` 하나를 사용한다.
- 월드 포스터 Canvas는 테이블 메시 최고점보다 약 1.1cm 높은 `(0, 3.54, 16)`에 평평하게 놓이며 기존 현상수배 포스터 프리팹 2장을 재사용한다. 두 포스터는 상대편 테이블 바깥에서 플레이어 방향으로 `0.9초 / OutCubic` 동시 이동한다. 포스터 로컬 스케일은 기존 `1.7`의 65%인 `1.105`, 최종 Y는 `0`으로 두어 16:9의 720p와 1080p에서 화면 높이 약 51%를 차지한다.
- 진입 순서는 `Current 카메라 안정화 → 포스터 이동 → TableTop 블렌드 완료 → 선택 허용`이다. 포스터 선택이 가능해진 뒤에도 카메라 입력 잠금은 유지하므로 선택 전까지 W/S와 `StepView`로 TableTop을 벗어날 수 없다. 같은 Offer의 반복 렌더는 연출을 재시작하지 않는다.
- 포스터 선택 시 추가 입력을 즉시 닫고 두 포스터를 `0.9초 / InCubic`으로 등장 위치까지 되돌리면서 알파를 1에서 0으로 낮춘다. 완전히 사라진 뒤 `Current`로 블렌드하고, 블렌드가 끝난 뒤에만 View 내부 확정 토큰을 열어 상대 선택을 커밋한다. 따라서 적 바인딩과 등장 연출은 포스터 퇴장·프론트 뷰 복귀보다 먼저 시작되지 않는다. 확정 실패·새 Offer·화면 비활성화에서는 포스터 알파를 1로 복원한다.
- 화면 이탈·비활성화·새 Offer 전환은 진행 중 Tween과 Coroutine을 정리하고 중첩 카메라 잠금을 정확히 한 번 반환한다. 일시정지에서는 기본 스케일 시간에 따라 연출과 블렌드가 함께 멈췄다가 재개된다.
- `GameFlowController`의 직접 상대 선택 API는 월드 View의 준비 상태를 계속 확인하므로 연출 중 우회 확정할 수 없다. 퇴장 완료 콜백만 현재 Offer·후보 키·화면·저장 입력 잠금을 다시 검증하는 내부 경로를 사용한다. 확정이 거절되면 같은 Offer의 진입 연출을 복구하고 다시 선택 가능하게 한다.
- 최종 보스는 별도 화면 포스터를 만들지 않고 같은 월드 View의 첫 슬롯을 X `0`에 중앙 배치하며 둘째 슬롯을 숨긴다. 일반 선택으로 돌아오면 중심 간격 `650`인 X `-325/+325`로 복원한다. `FinalBossReveal` 논리 화면과 확인 상태는 유지하되 진입·입력 잠금·클릭·퇴장·실패 복구 연출은 일반 상대 선택과 공유한다.
- 2026-08-10 검증에서 GSV20 6/6(job `46bceb26d2064bd4860bd7bf4e405abe`), `GameSceneFullFlowPresentationTests` 14/14(job `a5de1a9b5a0b4b3aaccdee4a5d387d9f`), `MoodControllerTests` 22/22(job `021dfd22a5b84e2f9716aea8ce2a04e5`)가 통과했다. GameScene validation은 issue 0건이다. 초기화 직후 Error는 0건이었으나 이후 병행 Play Mode에서 기존 머티리얼 Drawer 오류 2건이 재현됐고 보스 포스터 변경 스택은 없었다. 정식 런의 둘째 상점 종료부터 보스 전투 진입까지와 1280×720·1920×1080 육안 확인은 이번 검증에서 별도로 완료하지 않았다.
