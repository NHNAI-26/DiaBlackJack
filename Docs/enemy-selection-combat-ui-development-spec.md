...
### 1.1 Notion v0.7 재연동 계약

- 후보 생성은 일반 4종·엘리트 1종으로 유지하며 보스는 고정 진입한다.
- `EnemyProfilePreview.MaximumSoul`은 3·3·5·5·5·8 개정값을 카탈로그에서 그대로 투영한다.
- 후보 요약에는 겁쟁이 합 15·수동 카드 15%, 사기꾼 체인지, 집행자 배분 전 독극물·파이몬과 보스 영혼 5·2 계약 교체처럼 전투 전에 알아야 할 고유 규칙을 포함한다.
- 예상 골드는 프로필 키로 조회한 3·4·6·7·9·15를 표시하되 UI에 값을 복제하지 않는다.
- 전투 정보는 기존 공개 관측 경계를 유지하고 새 정책 상태를 읽기 전용 스냅샷으로만 받는다.
- 보스의 기존 `BossPhase` 표시는 새 계약 단계로 교체하고 전투 시작·영혼 5·2 단방향 단계를 안전 스냅샷에서 표시한다. UI가 영혼으로 단계를 다시 계산하지 않는다.
...
| 날짜 | 작성자 | 변경 내용 |
| --- | --- | --- |
| 2026-08-10 | Codex | 최종 보스 포스터에서는 조명·배경만 전환하고 문 연출은 승인 뒤 프론트뷰에서 시작하도록 분리했으며, 캐릭터 표시 상태 초기화와 `bossStage` BGM 재준비로 문→캐릭터→BGM 순서를 고정 |
