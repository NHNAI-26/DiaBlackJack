...
| 계약 UI·적 AI | 미구현 |
| 최근 자동 기준선 | DC-04 전체 EditMode 362/362 통과 기록 |
...
다음 작업은 **DC-05 플레이어 계약 화면·런 연결**이다. 기존 `PendingDemonContractInteraction`, 공개 가능한 활성 런타임 상태와 안전한 최근 결과를 GameScene 표시·입력에 연결하고, 독립·런 세션의 영혼·승패·보상 동기화를 검증한다.

후보 3장, 마몬 재굴림·최종 적용과 벨페고르 선택은 같은 상호작용 ID 입력을 사용하되 플레이어 전용 정보만 표시한다. 사탄은 아직 DC-06 결정 게이트에 있으므로 DC-05 화면에서 구현된 것처럼 노출하지 않는다.
