...
다음 작업은 **DC-02 영혼 비용·전투당 횟수·후보 3장·필수 선택 상태와 세션 전달**이다. DC-01의 `DemonContractDeck`을 사용하되 개별 악마 효과와 화면은 함께 구현하지 않는다.

DC-02에서는 계약 가능 여부 조회가 덱·영혼·난수를 변경하지 않게 하고, 비용 지불 전에 후보 3장 확보 가능성을 검증한다. 비용 지불 뒤에는 취소·환불을 막고 물리 카드 하나를 활성 위치, 나머지를 버린 더미로 보내는 원자적 선택 경계를 구현한다.
