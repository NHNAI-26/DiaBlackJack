...
| 최근 자동 기준선 | DC-R02 전용 12/12·CoreLoop 363/363·전체 EditMode 553/553 통과 |
...
다음 작업은 DC-R03이다. 마몬 재굴림을 일반 행동 소비로 옮기고, 레비아탄이 기존 리볼버의 정보 은닉을 유지하며 두 번 발동하도록 연속 입력·중단·대가 경계를 이관한다.

그다음 나머지 악마는 DC-R04의 수직 순서로 구현한다. 실제 시작 선택 UI, 런타임 생성 경로 교체와 사탄 월드 활성 계약 카드 입력·양면 연출은 DC-R05에서 함께 검증한다.
