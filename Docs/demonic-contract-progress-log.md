...
> 작업 식별자: DC-00~DC-07  
> 버전: v0.8
> 현재 단계: DC-06 사탄 규칙·권능·화면 검증 완료
> 다음 단계: DC-07 적 AI 계약·반복 회귀
> 최종 갱신: 2026-07-23
...
| 최근 자동 기준선 | DC-06 전체 EditMode 377/377 통과 기록 |
...
다음 작업은 **DC-07 적 AI 계약과 최종 반복 검증**이다. 광신도부터 공개 정보 전용 관측값으로 계약 후보를 평가하고, 실행 직전 영혼·횟수·후보 조건을 재검증하며, 사탄 괴력의 두 수 선언 같은 후속 선택도 별도 정책 경계로 전달한다.

플레이어 사탄 계약과 권능은 완료됐으므로 DC-07에서는 규칙을 다시 구현하지 않는다. 적 AI의 비공개 정보 접근 방지, 네 계약의 정상·거절·위험 경로 반복, 전투 종료 임시 카드 정리와 독립/런/보상/재시작 회귀를 마감한다.
