...
| 계약 데이터·덱 | 여섯 구현 정의·선택한 시작 악마·런 최초/현재 덱·전투 드로우/버림 덱·후보 1~2장 구현 완료 |
| 계약 공통 행동 | 비용·전투당 횟수·후보 필수 선택·활성/버림·세션 전달 구현 완료 |
| 계약 개별 효과 | 벨페고르·마몬·레비아탄·사탄·바알제붑·메피스토펠레스 구현 완료, 나머지 6종 미완료 |
...
- `IDemonContractBustReplacementHandler`를 사탄의 절대 버스트 방지 뒤에 배치했다. 바알제붑은 소유자 영혼 1을 먼저 차감하고 생존한 경우 양쪽에서 가장 최근에 추가된 공개 카드 1장씩을 기존 버린 카드 더미로 이동한다.
...
| DC-D12 | 2026-07-28 | 프로토타입 | 바알제붑은 양쪽에서 가장 최근에 추가된 공개 카드 1장씩을 버리며, 대가로 영혼 0이면 카드 이동 없이 즉시 사망 | 별도 대상 선택 문구가 없어 입력 교착을 만들지 않는 결정적 기준으로 DC-R04 1차 구현·회귀 고정 |
...
| DC-R04 1/5 | 8/8·400시드 | CoreLoop 377/377·전체 566/566 | 미실행, DC-R05 연결 범위 | 씬 에셋 무변경 | 컴파일 오류 0 | 바알제붑·메피스토펠레스 완료, 나머지 6종 미완료 |
...
다음 작업은 DC-R04 2/5 아스모데우스·아자젤이다. 차례 시작 상대 강제 히트 선택, 숫자 6 이하 카드 사용 제한, 사용 완료 공개 카드 재활성화와 히트 카드 효과 반복, 같은 숫자 공개 카드 유입 버스트를 소유자 방향 규칙으로 구현한다.
