...
다음 작업은 **DC-03 벨페고르 수직 기능**이다. DC-02의 계약 선택과 활성 계약을 사용해 히트 전 덱 위 한 장 미리보기, 그대로 히트 또는 덱 아래 이동, 상대 스탠드 뒤 자동 스탠드 예약과 정보 은닉을 구현한다.

벨페고르의 미리보기 값은 플레이어 전용 선택에만 제공하고 공용 로그·적 AI·상대 표시에는 넣지 않는다. 사탄, 마몬, 레비아탄과 계약 UI는 이번 단계에 함께 구현하지 않는다.
