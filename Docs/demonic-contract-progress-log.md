...
> 작업 식별자: DC-00~DC-08
> 버전: v1.2
> 현재 단계: DC-R00 최신 노션 기획 이관 완료, 개정 코드 미착수
> 다음 단계: DC-R01 시작 악마 선택·후보 1~2장 구현 및 회귀 검증
> 최종 갱신: 2026-07-27
...
다음 작업은 DC-R01이다. 시작 악마 2장 중 1장 선택과 일반 계약 후보 0·1·2장 경계를 먼저 구현해 이후 악마 효과가 기대하는 덱 크기와 저장 체크포인트를 고정한다.

그다음 DC-R02 사탄, DC-R03 마몬·레비아탄을 순차 이관하고, 나머지 악마는 DC-R04의 수직 순서로 구현한다. 각 단계는 기존 완료 테스트를 삭제하지 않고 과거 규칙 테스트를 새 기획 기대값으로 명시적으로 교체한다.
