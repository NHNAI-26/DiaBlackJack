...
> 작업 식별자: DC-00~DC-08
> 버전: v1.4
> 현재 단계: DC-R01 완료·DC-M01 계약 후 상시 패널 제거 완료
> 다음 단계: DC-R02 사탄 카운트 4·활성 계약 양면 이관
> 최종 갱신: 2026-07-28
...
| 계약 데이터·덱 | 네 정의·선택한 시작 악마·런 최초/현재 덱·전투 드로우/버림 덱·후보 1~2장 구현 완료 |
| 계약 공통 행동 | 비용·전투당 횟수·후보 필수 선택·활성/버림·세션 전달 구현 완료 |
| 계약 개별 효과 | 벨페고르·마몬·레비아탄·사탄 구현 완료 |
| 계약 UI·적 AI | 플레이어 UI와 광신도 적 계약 완료 |
| 최근 자동 기준선 | DC-R01 전용 14/14·CoreLoop 349/349·StageProgression 162/162·전체 EditMode 511/511 통과 |
...
| DC-D04 | 2026-07-27 | 개정 확정 | 레비아탄은 리볼버를 2회 발동하고 최종 실패 시 영혼 1을 잃음 | 기존 공개 합 22 후처리는 과거 구현, 연속 입력은 DC-R03에서 고정 |
...
| 위험 | 현재 대응 |
| --- | --- |
| 사탄 규칙·UI 수치 재불일치 | 카운트 4·소유자 차례 훅·활성 계약 양면을 DC-R02 코드와 표시 테스트에서 함께 갱신 |
...
| 레비아탄 2회 입력이 숨은 정보를 노출하거나 효과를 중복 종료 | 기존 리볼버 정보 경계를 재사용하고 첫 종료 뒤 연쇄를 중단 |
...
다음 작업은 DC-R02다. 과거 별도 사탄 권능 카드를 제거하고, 활성 사탄 계약 카드 자체에 종말 카운트 4와 소유자 차례 종료 양면 전환을 이관한다.

그다음 DC-R03 마몬·레비아탄을 순차 이관하고, 나머지 악마는 DC-R04의 수직 순서로 구현한다. 실제 시작 선택 UI와 런타임 생성 경로 교체는 DC-R05에서 함께 검증한다.
