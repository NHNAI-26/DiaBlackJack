...
> **현행 효과 변경(2026-08-09)**
> 독극물은 영혼 3 또는 현재 보유 영혼 전부를 담보로 걸고 승리 시 영혼을 5개로 돌려받는 문구를 사용한다. 부활초는 양쪽이 비용 없이 독립적으로 선택하며, 수락한 참가자만 자기 손패 전부를 버린 뒤 비공개 1장과 공개 1장을 다시 받는다. 아래 영혼 비용과 기존 `RestartRound` 기록은 과거 구현 증거다.
...
4. 지불로 영혼이 0이 되어도 즉시 전투를 끝내지 않고 현재 라운드 종료까지 사망 판정을 유예한다.
5. 현재 라운드 독극물 승리 회복 예약을 추가한다. 라운드 결과와 회복을 모두 적용한 뒤에도 영혼이 0일 때만 전투를 끝낸다.
6. 원본을 공개 손패에 유지하고 해당 숫자를 포함해 공개 합을 검사한다.

즉시 스탠드는 원래 행동의 추가 차례 소비가 아니다. 처리 완료 뒤 보존한 부모 continuation을 정확히 한 번 재개하고, 플레이어가 스탠드했다면 적이 일반 Stand와 동일하게 스탠드·버스트할 때까지 행동한 뒤 기존 최종 승부 경계를 재사용한다. GameScene은 승인된 플레이어 `StandNow`에 한해 행동 해골을 Stand 버튼으로 옮긴다.
...
플레이어와 적은 각각 하나의 결정을 가진다. 한쪽의 `Decline`은 다른 쪽의 `Redeal`을 막지 않는다. 소유자 결정을 먼저 받고 상대 결정을 이어서 받는다. 영혼 비용은 없으며 적은 자기 공개 합이 21 초과이거나 상대보다 3 이상 낮을 때 재배분한다.
...
3. `Redeal`을 고른 참가자의 손패만 자기 버린 더미로 옮기고 스탠드를 해제한다.
4. 해당 참가자에게 비공개 카드 1장과 공개 카드 1장을 배분한다.
5. 실제 재배분이 발생했다면 그 참가자에게 귀속된 라운드 전용 자동 상태를 정리하고 바깥 수동 카드 효과의 재개 여부를 결과에 기록한다.
6. 소유자가 실제로 재배분됐다면 원본도 전체 손패와 함께 버린다. 소유자가 재배분되지 않았다면 원본을 공개 손패에 유지한다.

양쪽 영혼은 선택 결과와 무관하게 불변이다. 재배분하지 않은 참가자의 기존 손패·스탠드 상태도 유지한다. 둘 다 거절하면 원본 부활초를 공개 손패에 유지하는 것 외 전투 상태를 바꾸지 않는다. 참가자별 재배분은 새 라운드를 시작하거나 라운드 번호를 증가시키지 않는다.
...
| 자동 효과의 영혼 0과 남은 선택 | 원칙적으로 즉시 전투 패배. 독극물 담보만 라운드 종료·승리 회복 뒤까지 유예 |
...
| 부활초 | 적 자신의 공개 상태가 불리하면 비용 없이 자기 손패 재배분, 상대 선택은 별도 정책 주체가 같은 기준으로 판단 |
...
| AC02-U04 | 담보 지불로 영혼 0이 되어도 부모 효과와 라운드를 완료한 뒤 0이면 패배한다 |
| AC02-U05 | 지불 뒤 라운드 승리만 5 회복하고 최대치를 넘지 않는다 |
| AC02-U06 | 패배·부활초 초기화에서는 회복하지 않는다 |
| AC02-U07 | 같은 라운드의 여러 예약이 중복 없이 해결된다 |
| AC02-U10 | 담보 지불로 영혼 0이 된 참가자가 라운드 승리 시 5를 회복하고 생존한다 |
| AC02-U11 | 담보 지불로 영혼 0이 된 참가자가 라운드 패배 후에도 0이면 사망한다 |
| AC02-U12 | 적도 담보 지불로 영혼 0이 된 뒤 라운드 승리 시 같은 규칙으로 회복한다 |
...
| ID | 검증 |
| --- | --- |
| ACRV03-U01 | 양쪽에 서로 독립적인 거절·무료 재배분 선택을 제공한다 |
| ACRV03-U01A | 플레이어가 상대 결정을 보기 전에 먼저 확정하고, 상대 결정 공개 뒤 양쪽 결과를 동시에 적용한다 |
| ACRV03-U02 | 플레이어만 수락하면 영혼 변화 없이 플레이어 손패만 변경한다 |
| ACRV03-U03 | 적만 수락하면 영혼 변화 없이 적 손패만 변경한다 |
| ACRV03-U04 | 둘 다 수락하면 영혼 변화 없이 각자 손패를 정확히 한 번 처리한다 |
| ACRV03-U05 | 둘 다 거절하면 원본을 공개 손패에 유지하며 영혼·기존 손패·라운드는 불변이다 |
| ACRV03-U06 | 영혼 1인 참가자도 사망이나 비용 없이 재배분한다 |
...
## 2026-08-04 AC-UI01 공통 버튼 기술 명세

- `GameHudCombatPrefabBuilder`는 선택 슬롯을 직접 조립하지 않고 `Assets/03. Prefabs/UI/DefaultButton.prefab`의 중첩 프리팹 인스턴스로 생성한다.
- `GameSceneCombatHudPresenter`는 `AutomaticCardChoiceKind`와 옵션 ID를 기준으로 일반 선택만 `BottomRight`에 투영한다.
- 독극물과 부활초의 모든 플레이어 선택, 회중시계 원본 폐기 여부는 우측 하단이다.
- 화염 방사기와 회중시계 재활성화의 카드 선택은 테이블 직접 입력을 유지하고 건너뛰기만 우측 하단이다.
- 거짓말 탐지기의 숫자 선언은 순환형 숫자 선택 UI를 유지한다.

## 2026-08-08 AC-UI02 자동 카드 결과 안내 수명

- 자동 카드 결과 안내는 소유자·상대 턴 종료로 삭제하지 않지만, 라운드 결과가 확정되어 카드 비교 연출을 게시하기 직전에 삭제한다. 다음 라운드로 이전 결과를 넘기지 않는다.
- 새 자동 카드 결과가 발생하면 기존 결과를 교체하고, 라운드가 계속되는 동안에는 다음 수락된 플레이어 공개 행동이 기록될 때 삭제한다. 거부된 입력은 결과를 유지한다.
- 선택 안내는 같은 `CombatPromptView`에서 우선 표시하되 결과를 소비하지 않으므로, 선택 안내가 끝나면 기존 자동 결과가 다시 표시된다.
- 플레이어 거짓말 탐지기 결과는 선언 숫자와 상대 비공개 카드의 `이상/미만` 비교만 표시한다. 실제 비공개 숫자와 상대 소유 탐지기의 비교 결과는 노출하지 않는다.
- 관련 집중 EditMode 13/13 통과. 전체 EditMode 1,231건 중 1,213건 통과, 잔여 18건은 기존 HUD 배선·도감·카드 콘텐츠·프리팹·상점 표시 회귀다. 컴파일 오류는 0이다.

## 2026-08-09 AC-UI03 집행자 독극물 주입 연출 순서

- 비활성 직렬화된 `PoisonInjectionAnnounceView`는 최초 `Play`가 활성화한 상태를 `Awake`에서 다시 끄지 않는다.
- 독극물 cue는 `Play`가 실제 Tween을 시작한 경우에만 소비한다. 참조·스프라이트·활성 계층 문제로 시작하지 못한 cue는 재시도할 수 있다.
- 1라운드와 후속 라운드 모두 독극물 페이드 인 `0.25초`, 홀드 `0.6초`, 낙하 `0.6초`가 끝난 뒤 양측 손패를 렌더한다.
- 손패 렌더 뒤에는 양측 카드 진입과 필요한 공개 플립이 끝날 때까지 합계와 다음 타임라인을 보류한다.
- 같은 프레임의 중복 표시 갱신은 기존 `CardHand` 인스턴스와 진행 중인 진입 Tween을 재사용한다.
