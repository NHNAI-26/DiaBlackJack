...
> 작업 식별자: AC-00~AC-06  
> 버전: v0.2
> 상태: AC-06 과거 효과 구현 보존 · 노션 v0.6 숫자·정의 키 이관 명세 수립
> 최종 갱신: 2026-07-27

> **현행 숫자 변경 안내**
> 목표 정의는 독극물 1, 부활초 2, 거짓말 탐지기 3, 화염 방사기 4, 회중시계 5다. 현재 코드의 `poison-2`, `flamethrower-9`, `pocket-watch-9`와 이를 참조하는 보상·적 덱·테스트는 AC-RV01에서 원자적으로 교체한다.
...
- 정의 키 변경은 저장 콘텐츠 리비전과 함께 처리한다. 과거 키를 새 키로 마이그레이션하지 않는 경우 이전 콘텐츠 리비전 저장은 명시적으로 거절하고 새 런을 안내한다.
