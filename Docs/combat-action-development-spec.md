...
> 문서 책임자: 이천서  
> 버전: v0.1  
> 상태: BA-05 런 연동·전체 검증 완료 기준
> 최종 갱신: 2026-07-19

> **현행 규칙 변경 안내 (2026-07-21)**
> 본 문서는 v0.1 구현 당시의 기술 기록이다. 현행 기획에서는 폴드를 삭제하고 체인지를 전투 내 누적 비용 `0, 1, 2, 3… 영혼` 방식으로 변경했으며, 플레이어의 스탠드 비교 패배는 영혼 1, 숫자·카드 효과 버스트는 영혼 2 피해로 분리했다. 아래 폴드 및 라운드당 1회 체인지 내용은 구현 이력 보존용이며, 실제 코드는 아직 현행 규칙으로 이관되지 않았다. 신규 구현 기준은 `Docs/combat-action-design.md` v0.2를 따른다.
