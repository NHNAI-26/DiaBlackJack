...
> 문서 책임자: 이천서  
> 버전: v0.3
> 상태: 공통 규칙 이관 완료 · 사기꾼 고정 비용 BA-R01 대기
> 최종 갱신: 2026-07-29
...
> **BA-R01 예외 (2026-07-29)**
> Notion v0.7의 사기꾼은 체인지 비용을 누적하지 않고 매회 영혼 1을 지불한다. 공통 `CompletedPlayerChangeCount` 의미는 유지하고, 적 프로필이 제공하는 비용 정책 또는 행동 후보 값으로 예외를 주입한다. `CoreLoopBattle`이 프로필 키 문자열을 직접 분기하지 않도록 하며 EP-R05와 함께 테스트한다.
