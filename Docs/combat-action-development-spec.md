...
> 문서 책임자: 이천서  
> 버전: v0.5
> 상태: BA-R01 적 공통 체인지·사기꾼 고정 비용 구현 완료
> 최종 갱신: 2026-07-29
...
> **BA-R01 적 체인지 (2026-07-29)**
> 모든 적은 자기 비공개 역할 카드 공개 또는 비공개 포함 전체 합 21 초과를 공통 발동 조건으로 사용한다. 공개 합 21 초과는 즉시 버스트 처리되어 체인지 행동 창이 없으므로 조건으로 쓰지 않는다. 후보는 21 이하 최고 합, 모두 초과면 최저 합, 동률이면 제시 순서로 고른다. 사기꾼만 비용을 매회 영혼 1로 고정한다. `CoreLoopBattle`이 프로필 키 문자열을 직접 분기하지 않도록 비용 모드를 주입했고, `StageBattleFactory`가 프로필 전투에만 기능을 활성화했다.

### 0.1 BA-R01 순수 도메인 계약

- 적 체인지 가능 여부는 적 자신의 손·영혼·덱 후보 수와 공개된 역할 상태만 읽는다.
- `EnemyObservation`에는 적이 선택할 자기 후보를 싣지 않는다. 실제 후보는 행동 승인 뒤 전투 내부에서 뽑아 적 전용 선택기로 해결한다.
- 시작 전 모든 조건과 비용을 검증하고, 실패하면 영혼·손·덱·완료 횟수·차례를 바꾸지 않는다.
- 기존 비공개 카드는 양측에 공개한 뒤 버리고 선택 후보만 새 뒷면 비공개 역할 카드가 된다.
- 프로필 강제행동이 있으면 `공개된 플레이어 비공개 카드 리볼버 대응 → 성공 전 광신도 계약 → 적 체인지 → 일반 점수 평가` 순서를 사용한다.
- 동일 시드·상태·후보 순서에서는 같은 선택을 반환한다.
...
| 날짜 | 작성자 | 변경 내용 |
| --- | --- | --- |
| 2026-07-29 | 이천서 | BA-R01 적 체인지 승인·비용·후보 선택·공개 행동 기록과 사기꾼 고정 비용을 구현하고 직접 생성 전투 호환 및 EP-R05 전체 655/655 검증 반영 |
