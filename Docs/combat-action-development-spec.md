...
> 문서 책임자: 이천서  
> 버전: v0.4
> 상태: 공통 규칙 이관 완료 · 적 공통 체인지·사기꾼 고정 비용 BA-R01 대기
> 최종 갱신: 2026-07-29
...
> 모든 적은 자기 비공개 역할 카드 공개 또는 비공개 포함 예상 최종 합 21 초과를 공통 발동 조건으로 사용한다. 공개 합 21 초과는 즉시 버스트 처리되어 체인지 행동 창이 없으므로 조건으로 쓰지 않는다. 후보는 21 이하 최고 합, 모두 초과면 최저 합, 동률이면 제시 순서로 고른다. 사기꾼만 비용을 매회 영혼 1로 고정한다. `CoreLoopBattle`이 프로필 키 문자열을 직접 분기하지 않도록 비용 정책을 주입하고 EP-R05와 함께 테스트한다.
