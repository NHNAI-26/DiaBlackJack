...
- `StageProgressionSession`: 현재 히트·스탠드 전달과 종료 결과 동기화, 폴드·체인지는 BA-05 대상

BA-01 기반 추가 후 CoreLoop 35/35, 전체 EditMode 58/58이 통과했다. BA-02 폴드 테스트 6개 추가 후 CoreLoop 41/41, 전체 64/64가 통과했다. BA-03 체인지 테스트 8개를 추가한 뒤 CoreLoop 49/49, 전체 EditMode 72/72가 통과했다. BA-04 착수 기준선 72/72를 다시 확인했고 표시·Controller 테스트 6개를 추가한 뒤 CoreLoop 55/55, 전체 EditMode 78/78이 통과했다.
...
BA-04에서는 `StageProgressionSession`의 행동 전달·종료 동기화를 아직 추가하지 않았다. 따라서 런 전투 Controller 입력 자체는 동작하지만 폴드로 전투가 끝날 때 런 패배와 지속 영혼을 동기화하는 책임은 BA-05에서 구현한다. 새 씬·패키지는 추가하지 않았다.
