...
> 새 런 시작 시 악마 2장 중 1장을 필수 선택해 악마 덱을 최소 1장으로 보장한다. 일반 계약 후보는 가용 카드 중 최대 2장이며 한 장뿐이면 단일 후보를 허용하고, 루시퍼만 5장 후보를 사용한다. DC-R01에서 순수 도메인과 저장 경계를 이관했고, 실제 기본 런 경로는 빈 악마 덱·선택 생성기·선택 직후 체크포인트를 사용한다. 아래의 4장 시작 덱·후보 3장·DC-01~DC-08 완료 기록은 과거 검증 기준이다.
...
- 선택 생성기를 주입하지 않은 테스트·호환 생성 경로만 네 장 프로토타입 덱을 유지한다. `StageProgressionRuntime.CreatePrototypeSession`은 빈 악마 덱과 `StartingDemonSelectionGenerator`를 주입한다.
...
- 실제 새 런은 선택 전 악마 덱 0장, 선택 뒤 1장이다. 독립 전투 화면의 12장 덱은 열두 계약을 빠르게 시험하는 개발용 하네스이므로 런 규칙으로 해석하지 않는다.
