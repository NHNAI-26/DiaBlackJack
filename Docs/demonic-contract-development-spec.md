...
DC-04 구현의 `IDemonContractAfterCardEffectHandler`는 완료된 `CardEffectResult`만 받는다. 원 리볼버의 `RoundResolution`이 있으면 계약 훅보다 먼저 종료하므로 판정을 복제하지 않는다. 실패 시에만 레비아탄 처리기가 내부 전체 합을 확인하고 안전한 `DemonContractEffectResult`와 선택적 계약 버스트 결과를 반환한다.
