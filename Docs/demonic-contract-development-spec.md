...
> 작업 식별자: DC-00~DC-08
> 버전: v1.11
> 상태: DC-R04 완료 — 악마 12종 규칙 구현, DC-R05 통합 검증 대기
> 최종 갱신: 2026-07-28

> **현행 기획 변경 안내 (2026-07-26)**
> 새 런 시작 시 악마 2장 중 1장을 필수 선택해 악마 덱을 최소 1장으로 보장한다. 일반 계약 후보는 가용 카드 중 최대 2장이며 한 장뿐이면 단일 후보를 허용하고, 루시퍼만 5장 후보를 사용한다. DC-R01에서 이 규칙의 순수 도메인과 저장 경계를 이관했다. 아래의 4장 시작 덱·후보 3장·DC-01~DC-08 완료 기록은 과거 검증 기준이며, 실제 시작 화면과 기본 런 생성 경로는 DC-R05 전까지 호환용 네 장 덱을 유지한다.
...
- 선택 생성기를 주입하지 않은 기존 생성 경로는 DC-R05 연결 전까지 네 장 프로토타입 덱을 유지한다.
