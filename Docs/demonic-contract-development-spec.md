...
- 벨페고르: 자동 스탠드 예약 여부
- 마몬: 현재 주사위 값, 현재 차례 재굴림 행동 완료 여부
- 레비아탄: 별도 가변 값 없음
...
- 첫 발동이 전투를 끝내면 두 번째 발동과 대가를 중단한다. 계속되면 두 번째 선언 입력을 새 상호작용 ID로 요청한다.
...
현재 `IDemonDieRoller`, 물리 계약별 상태와 `MammonApplyDie` 최종 보정은 유지한다. 기존 `MammonReroll` 차례 시작 보류는 일반 행동 명령으로 이관하며, 승인 결과가 적 차례 전환까지 원자적으로 완료되어야 한다. 보정 합은 에이스를 포함한 전체 손패와 함께 다시 계산한다.
...
기존 DC-04의 공개 합 22 이상 후처리는 제거 대상이다. 새 구현은 `AutoPistolEffectHandler`의 숨은 값 비교를 복제하지 않고, 전투 조정자가 두 번째 안전한 선언 상호작용을 요청하는 방식으로 구성한다.
...
- 레비아탄의 리볼버 2회 선언·첫 성공 중단·두 번 실패 대가와 숨은 값 미노출
