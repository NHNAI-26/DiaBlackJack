...
> 작업 식별자: DC-00~DC-08
> 버전: v1.13
> 상태: DC-R05 완료 — 활성 입력·후보 화면·저장 재진입 검증 통과
> 최종 갱신: 2026-07-28
...
- 리볼버 사용을 승인하면 같은 소유자·물리 카드·활성 계약을 묶은 연속 효과 컨텍스트에서 최대 두 번 처리한다.
- 첫 발동이 실제로 전투를 끝내면 두 번째 발동과 대가를 중단한다. 버스트 방지로 전투가 계속되면 두 번째 선언 입력을 새 상호작용 ID로 요청한다.
- 각 선언은 독립적이므로 첫 선언과 같은 숫자를 다시 선택할 수 있다.
- 두 번째 발동까지 상대가 생존하면 같은 카드 행동 안에서 소유자 영혼 1을 잃는다.
...
- 첫 발동이 상대를 버스트시키면 두 번째 발동과 영혼 대가를 실행하지 않는다.
- 두 번째 발동 뒤에도 상대가 생존하면 소유자 영혼 1을 차감하고 고갈 여부를 즉시 확인한다.
...
`LeviathanCardEffectSequence`는 소유자, 활성 계약 물리 ID, 리볼버 물리 ID와 첫 발동 성공 여부만 내부에 보존한다. 공개 `LeviathanCardEffectResult`는 1~2개의 성공 여부, 버스트 대상과 지불 영혼만 제공한다. 첫 발동이 전투를 끝내면 1회 결과로 종료하고, 두 번째 뒤 생존하면 기존 소유자 방향 계약 후처리가 영혼 1을 차감한다. 여러 레비아탄 계약의 연쇄 횟수는 미확정이므로 현재 Resolver는 기본 계약 제한 아래 첫 유효 계약 하나만 사용한다.
...
- 레비아탄의 리볼버 2회 선언·첫 전투 종료 중단·두 번째 뒤 상대 생존 대가와 숨은 값 미노출
