...
- 레비아탄: 별도 가변 값 없음
- 사탄: 남은 카운터, 권능 카드 ID, 최종 확정된 카운터 방식

여러 악마의 모든 필드를 하나의 거대한 상태 클래스에 넣지 않는다. 복수 계약은 후속 범위지만 컬렉션 형태로 읽을 수 있게 해 한 개 고정 필드에 대한 마이그레이션을 피한다.
...
선택 승인 뒤 즉시 효과가 플레이어를 버스트시켜도 계약 비용과 선택은 되돌리지 않는다. 이는 정상적인 계약 위험 결과다.
...
- 레비아탄: 명시적인 계약 직접 효과에 한해 전체 합 내부 조회
...
- 실패 뒤 상대 전체 손패 합이 22 이상이면 `ContractEffectBust`로 종료한다.
- 그렇지 않으면 소유자 영혼 1을 차감하고 고갈 여부를 즉시 확인한다.
- 결과 모델에는 `Triggered`, `BustedTarget`, `PaidSoulCost`만 남기고 숨은 합은 넣지 않는다.

### 9.4 사탄

사탄은 DC-D05·D06 확정 전 처리기를 등록하지 않는다. 부분 등록이나 임시 기본값은 UI와 테스트가 잘못된 규칙을 정식 기능으로 간주하게 하므로 금지한다.
...
플레이어 계약 1차 범위가 안정되기 전에는 `CultistEnemyPolicy`에 가짜 계약 후보를 추가하지 않는다.
...
- 레비아탄의 기존 리볼버 성공, 추가 성공, 최종 실패 대가와 숨은 합 미노출
...
- DC-01~DC-07의 대상 테스트와 전체 EditMode가 모두 통과한다.
