...
> 작업 식별자: DC-00~DC-08
> 버전: v1.2
> 상태: DC-08 과거 구현 보존, 노션 v0.6의 후보·사탄·마몬·레비아탄 개정 이관 명세 수립
> 최종 갱신: 2026-07-27

> **현행 기획 변경 안내 (2026-07-26)**
> 새 런 시작 시 악마 2장 중 1장을 필수 선택해 악마 덱을 최소 1장으로 보장한다. 일반 계약 후보는 가용 카드 중 최대 2장이며 한 장뿐이면 단일 후보를 허용하고, 루시퍼만 5장 후보를 사용한다. 아래의 4장 시작 덱·후보 3장·DC-01~DC-08 완료 기록은 현재 코드와 과거 검증 기준이며 새 기획으로 코드 이관하기 전까지 현행 플레이 규칙으로 간주하지 않는다.
...
현행 목표 API는 가용 카드 수를 먼저 확인해 `min(2, AvailableCardCount)`장을 이동한다. 가용 카드가 1장이면 단일 후보를 만들고 0장이면 비용·횟수·난수 상태를 바꾸지 않고 거절한다. `CandidateCount = 3` 상수와 정확히 3장을 요구하는 전투 검사는 제거하거나 일반 계약의 `MaximumCandidateCount = 2`와 루시퍼 전용 5장 요청으로 분리한다.
