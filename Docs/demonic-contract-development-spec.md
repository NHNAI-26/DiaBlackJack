...
DC-R01 구현은 가용 카드 수를 먼저 확인해 `min(2, AvailableCardCount)`장을 이동한다. 가용 카드가 1장이면 단일 후보를 만들고 0장이면 비용·횟수·난수 상태를 바꾸지 않고 거절한다. 일반 계약은 `TakeCandidates()`와 `MaximumCandidateCount = 2`, 루시퍼 예외는 내부 `TakeLuciferCandidates()`와 `LuciferCandidateCount = 5`로 분리했으며 현재 루시퍼 효과 자체는 DC-R04 범위다.
...
- 네 우선 악마의 최신 표시 문구·효과·대가·발동 순서가 일치한다.
