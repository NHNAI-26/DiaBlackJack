...
> 작업 식별자: DC-00~DC-08
> 버전: v1.7
> 상태: DC-R04 진행 중 — 바알제붑·메피스토펠레스 완료, 아스모데우스·아자젤 대기
> 최종 갱신: 2026-07-28
...
- `DemonContractContext.TryPayOwnerSoulAndDiscardLatestFaceUpCards`는 양쪽 공개 카드 존재를 모두 검증한 뒤 영혼 1을 먼저 차감한다. 소유자가 생존하면 양쪽의 가장 최근 공개 카드 ID를 기존 `TryDiscardCard`로 이동하고, 영혼 0이면 카드 이동 없이 즉시 전투 종료 경계로 반환한다.
...
- 카탈로그와 기본 Resolver는 여섯 구현 악마를 제공한다. 광신도는 바알제붑의 추가 영혼 대가를 생존할 수 있는지와 미사용 보위 나이프 보유 여부만 관측해 선택 점수를 매긴다.
...
`StageBattleFactory`는 광신도 프로필에 현재 구현된 여섯 장의 전투별 악마 덱을 주입한다. 적 활성 계약은 플레이어와 분리된 컬렉션으로 보존하되 동일한 `DemonContractResolver` 처리기를 소유자 방향만 바꾸어 사용한다. 벨페고르 미리보기, 마몬 차례·최종 선택, 레비아탄 카드 후속 훅, 사탄 스탠드·버스트·카운터·양면 상태, 바알제붑 버스트 대체와 메피스토펠레스 비공개 공개를 적 소유자에도 같은 규칙으로 적용한다.
